...
팔은 사용자 지정 캔 시작 위치 XY=(0.55,0), yaw=-90°의 world 목표를 사용하고,
...
55cm는 로봇 베이스 중심으로부터 초기 캔 pose 원점까지의 수평 거리입니다.
`config/arm_ground_frame.json`의 고정 변환으로 손·캔 궤적을 함께 옮기므로 상대 동작은 유지합니다.
원본 데이터나 floating 정책의 학습 좌표는 변경하지 않습니다. 팔 배치를 바꾸면 저장된 IK reference를 다시 생성합니다.
