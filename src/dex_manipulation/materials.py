...
PAD_BODIES = tuple('right_'+finger+'_touch_link' for finger in ('thumb','index','middle','ring','pinky'))
...
def bind_pad_material(stage, robot_path, config, material_path):
    ...
    bodies = [p for p in Usd.PrimRange(root) if p.HasAPI(UsdPhysics.RigidBodyAPI) and p.GetName() in PAD_BODIES]
    ...
    for body in bodies:
        ...
        UsdShade.MaterialBindingAPI.Apply(body).Bind(material, UsdShade.Tokens.strongerThanDescendants, 'physics')
    ...
    return dict(profile=settings, material_path=material_path, pad_colliders=paths,
...
def pad_shape_mask(env):
    ...
    if not hasattr(env, 'body_shape_counts'):
        env.body_shape_counts = [env.world.physics_sim_view.create_rigid_body_view(path).max_shapes
    ...
    return torch.tensor([name in PAD_BODIES for name,count in zip(env.robot.body_names,counts)
                         for _ in range(count)], dtype=torch.bool)
...
def enforce_pad_contact(env):
    ...
    if pads.count != 5*env.num_envs or pads.max_shapes != 1:
        raise ValueError('Pad-only PhysX view did not isolate five single-shape bodies per environment')
    ...
    properties = torch.tensor([settings['static_friction'],settings['dynamic_friction'],
                               settings['stiffness_n_m'],settings['damping_ns_m']]).expand(pads.count,1,4).contiguous()
    modes = torch.zeros((pads.count,1,3),dtype=torch.uint8)
    pads.set_compliant_material_properties(properties, modes, torch.arange(pads.count,dtype=torch.int32))
    ...
    if not torch.allclose(friction, torch.tensor([settings['static_friction'],settings['dynamic_friction']]).expand(env.num_envs,5,2)):
...
def pad_contact_report(env):
    ...
    report = dict(settings=env.cfg['contact_materials'], pad_shape_count=int(mask.sum()),
                  other_robot_shape_count=int((~mask).sum()), groups={})
    for label, selected in (('pads',mask),('other_robot',~mask)):
        report['groups'][label] = dict(
            static_friction_range=[float(material[:,selected,0].min()),float(material[:,selected,0].max())],
            dynamic_friction_range=[float(material[:,selected,1].min()),float(material[:,selected,1].max())],
            stiffness_n_m=float(env.cfg['contact_materials']['stiffness_n_m']) if label=='pads' else None,
    ...
    for label, rigid in (('can',env.can),('table',env.table)):
        ...
        report['groups'][label] = dict(stiffness_n_m=0., rigid_contact=True,
