"""RB3 and Revo2 device adapters, calibration and read-only feedback probes."""
...
import time
import numpy as np
from dex_manipulation.robot.trajectory import sha256, stream
from dex_manipulation.sensors.hardware_tactile import decode_sdk_sample
from dex_manipulation.sensors.session import TactileCSV
...
async def execute(trajectory, config, output, hold_s, *, record_tactile=False, tactile_hz=100.0):
...
async def probe(config, output, seconds=5.0, record_tactile=False):
    transport_plan(config)
    ...
    try:
        ...
        device = await asyncio.wait_for(
            sdk.modbus_open(revo["port"], getattr(sdk.Baudrate, revo["baudrate_enum"])), 2.0
        )
        metadata.update(
            hand_type=str(await call("get_hand_type")),
        ...
        if record_tactile:
            if (
                not await call("is_touch_hand")
                or int(await call("get_touch_sensor_enabled")) & 31 != 31
            ):
        ...
        with (output / "raw_feedback.jsonl").open("x") as log:
            while time.monotonic() - start < seconds:
                ...
                if arm is None:
                    raise TimeoutError("RB3 feedback timeout")
                state = dict(
                    time_s=(before + after) / 2 - start,
                    read_window_s=after - before,
                    rb3_joint_deg=list(map(float, arm.sdata.jnt_ang)),
                    rb3_device_time_s=float(arm.sdata.time),
                    rb3_status=rb_status(arm.sdata),
                ...
                count += 1
                await asyncio.sleep(max(0.0, 0.1 - (time.monotonic() - cycle_started)))
