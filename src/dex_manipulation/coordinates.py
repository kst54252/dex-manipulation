...
def z_aligned_cylinders(geometry):
    ...
    if not shapes or any(
        s["type"] != "cylinder" or not np.allclose(np.asarray(s["transform"])[:3, :3], np.eye(3))
        for s in shapes
    ):
...
def transform_reference(data, frame):
    ...
    result["wrist_quaternion_xyzw"] = Rotation.from_matrix(
        result["wrist_transform"][:, :3, :3]
    ).as_quat()
    return result
...
def stabilize_grounded_dataset(poses_path, reference_path, geometry_path, output):
    ...
    rotation = (
        Rotation.from_rotvec(cross / np.linalg.norm(cross) * tilt).as_matrix()
    ...
    shifts[:, 2] = np.maximum(-bottoms, 0.0)
    ...
    metadata.update(
        world_from_source=total_frame.tolist(),
        source_from_world=inverse(total_frame).tolist(),
        ...
        hand_object_relative_transform_max_error=float(
            np.abs(relative_after - relative_before).max()
        ),
        ...
        sources=[
            dict(path=str(p), sha256=hashlib.sha256(p.read_bytes()).hexdigest()) for p in paths
        ],
        floor_stabilization=dict(
            fixed_new_from_previous=frame.tolist(),
            initial_tilt_before_deg=float(np.degrees(tilt)),
            frame_ids=reference["frame_ids"].tolist(),
            post_frame_translation_m=shifts.tolist(),
...
def ground_dataset(poses_path, reference_path, geometry_path, config_path, output):
    ...
    if "frame_metadata_json" in poses or "frame_metadata_json" in reference:
        raise ValueError(
            "Already transformed input: use the original camera dataset to avoid double grounding"
        )
    ...
    shapes = z_aligned_cylinders(geometry)
    if (
        geometry.get("fingerprint")
        and str(reference.get("object_geometry_fingerprint", "")) != geometry["fingerprint"]
    ):
    ...
    if mode == "initial_object_plus_z":
        ...
        assumption = (
            "Initial can local +Z is simulation up; no measured table/camera-to-robot calibration"
        )
        ...
        assumption = (
            "Declared camera -Y is simulation up; no measured table/camera-to-robot calibration"
        )
    else:
        raise ValueError("Unknown explicit grounding_mode")
    frame[2, 3] -= collision_bottom(frame @ first, shapes)
    grounded = transform_reference(reference, frame)
    orientation = None
    if config.get("object_orientation_requirement") == "initial_base_below_body":
        ...
        if not orientation["initial_base_below_body"]:
            raise ValueError(
                "Can wider base is above the body; fix mesh_to_object and rerun retargeting"
            )
    poses["pose_y"] = (frame @ source_poses)[..., :3, :]
    ...
    relative_after = np.linalg.inv(grounded["object_transform"]) @ grounded["wrist_transform"]
    metadata = dict(
        schema="dex_dataset_frame_v1",
        coordinate_frame="ground",
        length_unit="m",
        quaternion_order="xyzw",
        ground_z_m=0.0,
        initial_frame_id=int(poses["frame_ids"][0]),
        final_frame_id=int(poses["frame_ids"][-1]),
        frame_count=len(poses["frame_ids"]),
        frame_ids=poses["frame_ids"].tolist(),
        frame_range=config.get("frame_range"),
        duration_s=float(reference["timestamps_s"][-1] - reference["timestamps_s"][0]),
        source_axes=config["camera_axes"],
        world_axes=world_axes,
        world_from_source=frame.tolist(),
        source_from_world=inverse(frame).tolist(),
        object_index=config["object_index"],
        object_name=config["object_name"],
        initial_object_pose=grounded["object_transform"][0].tolist(),
        initial_collision_bottom_z_m=collision_bottom(grounded["object_transform"][0], shapes),
        collision_shapes=shapes,
        object_geometry_fingerprint=geometry.get("fingerprint"),
        hand_object_relative_transform_max_error=float(
            np.abs(relative_before - relative_after).max()
        ),
        time_basis=config.get("time_basis"),
        retimed_fps=config.get("retimed_fps"),
        ...
        sources=[
            dict(path=str(p), sha256=hashlib.sha256(p.read_bytes()).hexdigest()) for p in inputs
        ],
    )
    if mode != "initial_object_plus_z":
        metadata["grounding_mode"] = mode
    ...
    if config.get("source_playback_fps") is not None:
        metadata["source_playback_fps"] = config["source_playback_fps"]
    encoded = np.array(json.dumps(metadata))
    poses["frame_metadata_json"] = encoded
    grounded["frame_metadata_json"] = encoded
    output.mkdir(parents=True, exist_ok=True)
