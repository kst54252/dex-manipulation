...
def main():
    parser = argparse.ArgumentParser(description="Independent floating-hand interaction-mesh retargeting")
    ...
    p.add_argument("--direction-weight", type=float, default=0.5, help="Additional semantic finger-direction fidelity weight")
    p.add_argument("--position-weight", type=float, default=200.0, help="Additional semantic point squared-error weight, 1/m")
    p.add_argument("--wrist-linear-limit", type=float, default=1.0, help="Algorithmic floating wrist bound, m/s")
    p.add_argument("--wrist-angular-limit", type=float, default=8.0, help="Algorithmic floating wrist bound, rad/s")
    p.add_argument("--input", required=True, type=Path)
    p.add_argument("--metadata", required=True, type=Path)
    p.add_argument("--geometric-debug", action="store_true", help="Allows missing timing; all valid flags remain false")
    view = sub.add_parser("view")
    view.add_argument("--result", required=True, type=Path)
    view.add_argument("--model-dir", required=True, type=Path)
    view.add_argument("--reference-dir", type=Path, help="Original RGB/label directory for synchronized camera comparison")
    ...
    if args.command == "view":
        ...
        comparison_viewer(args.result / "trajectory.npz", args.result / "comparison.html", args.result / "report.json", model,
                          args.model_dir / "can_mesh.npz", args.reference_dir)
        return
    ...
    options = SolverOptions(laplacian_norm=args.norm, max_iterations=args.max_iterations,
                            finger_direction_weight=args.direction_weight, keypoint_position_weight=args.position_weight,
                            wrist_linear_velocity_m_s=args.wrist_linear_limit,
                            wrist_angular_velocity_rad_s=args.wrist_angular_limit)
    report = solve_sequence(model, sequence, points, scene, args.output, options, debug, args.max_frames)
    print(json.dumps({k: report[k] for k in ("classification", "frame_count", "valid_count", "failed_frame_ids")}))
