...
class JointCalibration:
    def __init__(self, names, config):
        ...
        for name in names[6:]:
            ...
            if (q.ndim != 1 or len(q) < 2 or u.shape != q.shape or not np.isfinite(q).all()
                    or not np.isfinite(u).all() or not (np.diff(q)>0).all()
                    or not ((np.diff(u)>0).all() or (np.diff(u)<0).all()) or u.min()<0 or u.max()>1000):
                raise ValueError('Revo2 requires measured monotonic rad <-> 0..1000 calibration knots')
    ...
    def encode(self, q):
        ...
        for value, (index, knots, units) in zip(q[6:], self.fingers):
            if not knots[0]-1e-7 <= value <= knots[-1]+1e-7:
                raise ValueError('Finger target outside calibrated range; no clipping/extrapolation')
...
def hardware_plan(trajectory, config):
    ...
    if (np.any(arm < lower) or np.any(arm > upper) or np.any(np.abs(arm)>360)
            or np.any(np.abs(velocity)>vmax) or np.any(np.abs(acceleration)>amax)):
        raise ValueError('Recorded speed/acceleration exceeds physical RB3 limits; speed will not be changed silently')
    ...
    if (not np.isfinite(values).all() or values[0]<.002 or not .02<values[1]<.2
            or values[2]<=0 or not 0<values[3]<1 or not np.isclose(values[0], trajectory.dt)):
        raise ValueError('Commission RB3 Servo J parameters; t1 must match the recorded 30Hz interval')
    validation = json.loads(Path(config['simulation_validation']).read_text())
    if (validation['commands_sha256'] != sha256(trajectory.path) or not validation['simulated_grasp']
    ...
    return dict(**trajectory.summary(), backend='rbpodo Servo J + bc-stark-sdk RS485',
                calibration_id=calibration['id'], rb3_initial_deg=arm[0].tolist(),
                revo2_initial_units=hand[0].tolist(), rb3_peak_velocity_deg_s=np.abs(velocity).max(0).tolist(),
...
class RBPodoStark:
    ...
    async def connect(self):
        ...
        self.hand = await asyncio.wait_for(stark.modbus_open(hand['port'], getattr(stark.Baudrate, hand['baudrate_enum'])), 2.)
    ...
    async def call_hand(self, method, *args):
        ...
        return (await asyncio.wait_for(value, self.config['io_timeout_s'])
                if inspect.isawaitable(value) else value)

    async def read(self):
        started = time.monotonic()
        arm, hand = await asyncio.gather(asyncio.to_thread(self.data.request_data, self.config['io_timeout_s']),
                                         self.call_hand('get_motor_status'))
        if arm is None: raise TimeoutError('RB3 feedback timeout')
        s = arm.sdata
    ...
    async def send(self, q, arm_velocity, dt):
        ...
        def arm_command():
            ...
            result = self.robot.move_servo_j(rc, arm, p['t1_s'], p['t2_s'], p['gain'], p['alpha'],
                                            self.config['io_timeout_s'], True)
            if not result.is_success(): raise RuntimeError('RB3 rejected Servo J or did not acknowledge')
        # Mark before sending: a partial two-device failure still needs stopping.
        self.sent = True
        results = await asyncio.gather(asyncio.to_thread(arm_command),
                                      self.call_hand('set_finger_positions', fingers.tolist()), return_exceptions=True)
        ...
        for result in results:
            ...
            if result is False: raise RuntimeError('Revo2 rejected finger command')
    ...
    async def stop(self):
        ...
        if self.robot is not None:
            calls.append(asyncio.to_thread(self.robot.task_stop, self.rb_sdk.ResponseCollector(), self.config['io_timeout_s'], True))
        ...
        if any(isinstance(r, BaseException) or r is False or (hasattr(r,'is_success') and not r.is_success()) for r in results):
            raise RuntimeError('Stop/hold acknowledgment failed; use the physical stop')

    async def close(self):
        try:
            if self.hand is not None:
                await asyncio.wait_for(self.hand_sdk.modbus_close(self.hand), self.config['io_timeout_s'])
        finally:
...
async def execute(trajectory, config, output, hold_s):
    ...
    guard = config['guard']
    output = Path(output)
    if output.exists(): raise ValueError('Use a new hardware output directory')
    try:
        return await stream(trajectory, RBPodoStark(trajectory.names, config), output,
            ...
            maximum_lateness_s=guard['maximum_lateness_s'], hold_s=hold_s)
    finally:
        if output.is_dir():
