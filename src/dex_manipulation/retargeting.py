...
def solve_sequence(model, sequence, object_points, collision_scene, output_dir, options=None,
                   geometric_debug=False, max_frames=None):
    options = options or SolverOptions()
    if min(options.smoothness, options.collision_margin_m, options.finger_direction_weight, options.keypoint_position_weight) < 0:
    ...
    frame_count = len(sequence.frame_ids) if max_frames is None else min(max_frames, len(sequence.frame_ids))
    if frame_count < 1:
    ...
    palm_indices = [semantic.index(s) for s in ("wrist", "index_mcp", "middle_mcp", "ring_mcp", "little_mcp")]

    for frame in range(frame_count):
        started = time.perf_counter()
        if not sequence.valid[frame]:
            records.append(dict(frame_id=int(sequence.frame_ids[frame]), valid=False, reason="invalid source annotations"))
        ...
        try:
            ...
            records.append(dict(frame_id=int(sequence.frame_ids[frame]), valid=False, reason=f"Delaunay failed: {error}"))
            graph_records.append([])
            continue
        ...
        if previous is None:
            ...
            dt = None if sequence.times is None else float(sequence.times[frame] - sequence.times[previous_index])
        ...
        alphas = np.array([1.0]) if previous is None else np.arange(1, options.collision_substeps + 1) / options.collision_substeps
        ...
        for alpha in alphas:
            if previous is None:
                ...
                delta_rotation = Rotation.from_matrix(can_pose[:3, :3] @ old_object[:3, :3].T).as_rotvec()
                collision_object_poses.append(transform(
                    Rotation.from_rotvec(alpha * delta_rotation).as_matrix() @ old_object[:3, :3],
                    (1 - alpha) * old_object[:3, 3] + alpha * can_pose[:3, 3]))

        def kinematics(x, alpha=1.0):
            wrist = transform(Rotation.from_rotvec(alpha * x[3:6]).as_matrix() @ base_rotation,
        ...
        def evaluate(x):
            ...
            if previous is not None:
                smoothing = options.smoothness * (
                    ...
                    + np.sum(x[3:6] ** 2) + np.sum((x[6:] - q0) ** 2))
            ...
            if collision_scene is not None:
                for alpha, object_pose in zip(alphas, collision_object_poses):
                    ...
                    sample_distance, witnesses = collision_scene.distances(sampled_links, object_pose, with_witnesses=True)
                    distances.extend(sample_distance)
                    collision_samples.append((alpha, sampled_links, sample_distance, witnesses))
            distances = np.array(distances)
            cached_x = x.copy()
            cached = (energy + smoothing + direction_energy + position_energy, energy, smoothing, residual, distances, wrist, points, links, collision_samples)
            return cached
        ...
        def inequalities(x):
            ...
            if dt is not None:
                result.extend([
                    options.wrist_linear_velocity_m_s - np.linalg.norm(x[:3] - pose0[:3, 3]) / dt,
        ...
        def inequality_jacobian(x):
            ...
            if len(distances):
                ...
                for sample_index, (alpha, links, sample_distance, witnesses) in enumerate(collision_samples):
                    ...
                    local_points = [links[name][:3, :3].T @ (point - links[name][:3, 3])
                                    for name, point in zip(names, witnesses[:, 0])]
                    jac[rows, :3] = alpha * normals
                    for column in range(3, len(x)):
                        offset = np.zeros_like(x)
                        offset[column] = 1e-6
                        plus, minus = kinematics(x + offset, alpha)[1], kinematics(x - offset, alpha)[1]
                        point_velocity = np.array([(apply(plus[name], point) - apply(minus[name], point)) / 2e-6
        ...
        for attempt in range(options.max_attempts):
            result = minimize(lambda x: evaluate(x)[0] / objective_scale, seed, method="SLSQP", bounds=bounds,
                              constraints=constraints, options=dict(maxiter=options.max_iterations,
                              ftol=options.ftol, eps=1e-6, disp=False))
            ineq = inequalities(result.x)
            minimum = float(ineq.min()) if len(ineq) else None
            attempts.append(dict(success=bool(result.success), status=int(result.status), message=str(result.message),
                                 iterations=int(result.nit), objective=float(result.fun * objective_scale), minimum_inequality=minimum))
            if result.success and (minimum is None or minimum >= -options.feasibility_tolerance):
                break
            if not np.isfinite(result.x).all():
                break
            seed = result.x.copy()
        objective, energy, smoothing, residual, distances, wrist, points, links, _ = evaluate(result.x)
        q = result.x[6:]
        ...
        if dt is not None:
            ...
            wrist_angular_speed = float(Rotation.from_matrix(wrist[:3, :3] @ pose0[:3, :3].T).magnitude() / dt)
            velocity_violation = float(max(0, np.max(speed - model.full_velocity),
                                           wrist_linear_speed - options.wrist_linear_velocity_m_s,
                                           wrist_angular_speed - options.wrist_angular_velocity_rad_s))
        separation = None if not len(distances) else float(distances[-len(collision_scene.pair_names):].min())
        path_separation = None if not len(distances) else float(distances.min())
        collision_flags = None if collision_scene is None else collision_scene.collision_flags(links, can_pose)
        feasible = (violation <= options.feasibility_tolerance
                    and velocity_violation <= options.feasibility_tolerance
                    and (path_separation is None or path_separation >= options.collision_margin_m - options.feasibility_tolerance)
        ...
        wrists[frame], active[frame], full[frame], robot_points[frame] = wrist, q, model.expand(q), points
        # Debug trajectories can be inspected but are never marked fully valid.
        valid[frame] = accepted and not geometric_debug
        records.append(dict(frame_id=int(sequence.frame_ids[frame]), valid=bool(valid[frame]),
                            optimizer_success=bool(result.success), feasible=bool(feasible), status=int(result.status),
                            message=str(result.message), iterations=sum(a["iterations"] for a in attempts),
                            attempts=attempts, elapsed_s=time.perf_counter() - started,
                            objective=float(objective), initial_objective=float(initial_objective),
                            objective_scale=objective_scale,
                            laplacian_energy=float(energy), smoothness_energy=float(smoothing),
                            finger_direction_energy=float(options.finger_direction_weight * np.sum(
                                (directions(points, articulation_edges) - source_directions) ** 2)),
                            keypoint_position_energy=float(options.keypoint_position_weight * np.sum((points - human) ** 2)),
                            ...
                            joint_limit_violation_rad=violation, velocity_limit_violation=velocity_violation,
                            full_joint_max_speed_rad_s=full_speed_max,
                            wrist_linear_speed_m_s=wrist_linear_speed, wrist_angular_speed_rad_s=wrist_angular_speed,
                            min_collision_distance_m=separation,
                            collision_distances_m=[] if collision_scene is None else distances[-len(collision_scene.pair_names):].tolist(),
                            path_min_collision_distance_m=path_separation, path_collision_alphas=alphas.tolist(),
                            path_collision_distances_m=distances.tolist(),
                            collision_flags=None if collision_flags is None else collision_flags.tolist(),
                            dt_s=dt, temporal_reference_frame=None if previous_index is None else int(sequence.frame_ids[previous_index])))
        print(f"[retarget] {sequence.frame_ids[frame]} success={result.success} feasible={feasible} "
              f"E={energy:.5g} gap={separation} iter={result.nit}", flush=True)
    ...
    for i, wrist in enumerate(wrists):
        if np.isfinite(wrist).all():
            ...
            if i and np.isfinite(quaternions[i - 1]).all() and np.dot(quaternions[i], quaternions[i - 1]) < 0:
                quaternions[i] *= -1
    frame_arrays = {}
    if 'frame_metadata' in sequence.metadata:
        frame_arrays['frame_metadata_json'] = np.array(json.dumps(sequence.metadata['frame_metadata']))
    np.savez_compressed(output_dir / "trajectory.npz", frame_ids=sequence.frame_ids[:frame_count],
                        timestamps_s=np.full(frame_count, np.nan) if sequence.times is None else sequence.times[:frame_count],
                        wrist_transform=wrists, wrist_translation_m=wrists[:, :3, 3], wrist_quaternion_xyzw=quaternions,
                        active_joint_names=np.asarray(model.active_names), full_joint_names=np.asarray(model.full_names),
                        active_q_rad=active, full_q_rad=full, semantic_names=np.asarray(model.semantic_names),
                        human_keypoints=sequence.hand[:frame_count], robot_keypoints=robot_points,
                        object_points_local=object_points, object_keypoints=object_world,
                        object_transform=sequence.object_poses[:frame_count], valid=valid, transition_valid=transition_valid,
                        object_geometry_fingerprint=np.array('' if collision_scene is None else collision_scene.fingerprint),
                        **frame_arrays)
    report = dict(classification="geometric_debug" if geometric_debug else "constrained_geometric_retargeting",
                  options=asdict(options), sequence_metadata=sequence.metadata, frames=records,
                  frame_count=frame_count, valid_count=int(valid.sum()), failed_frame_ids=sequence.frame_ids[:frame_count][~valid].tolist(),
                  collision_pairs=[] if collision_scene is None else collision_scene.pair_names,
                  object_geometry_fingerprint=None if collision_scene is None else collision_scene.fingerprint,
                  ...
                  not_validated=["continuous-time collision", "hand self-collision", "table/environment collision", "physical grasp", "dynamics", "RB3 IK"],
                  graph=dict(vertex_order="semantic robot order, then fixed object sample order", source_edges=graph_records))
