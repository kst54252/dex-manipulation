...
class RecordedCommands:
    def __init__(self, path):
        ...
        m = self.metadata
        if (m['schema'] != SCHEMA or m['angle_unit'] != 'rad' or
                m['command_semantics'] != 'start_of_interval_zero_order_hold'):
        ...
        for key, shape in [('q_command_rad', (n, 12)), ('q_arm_velocity_rad_s', (n, 6)),
                           ('initial_q_rad', (12,)), ('lower_rad', (12,)),
                           ('upper_rad', (12,)), ('velocity_limit_rad_s', (12,))]:
            if d[key].shape != shape or not np.isfinite(d[key]).all():
                raise ValueError(f'Invalid {key}')
        if (not np.isfinite([self.dt, self.duration]).all() or self.dt <= 0 or
                not np.allclose(self.times, np.arange(n)*self.dt, atol=1e-8, rtol=0) or
                not np.isclose(self.duration, n*self.dt, atol=1e-8, rtol=0)):
        ...
        if d['ik_success'].shape != (n,) or d['ik_success'].dtype.kind != 'b' or not d['ik_success'].all():
            raise ValueError('Recorded trajectory contains an IK failure')

    def export_csv(self, path):
        header = ['timestamp_s', *[n+'_rad' for n in self.names],
                  *[n+'_velocity_rad_s' for n in self.names[:6]]]
        np.savetxt(path, np.c_[self.times, self.q, self.data['q_arm_velocity_rad_s']],
                   delimiter=',', header=','.join(header), comments='')

    def summary(self):
        return dict(command_count=len(self.times), reference_duration_s=self.metadata['reference_duration_s'],
                    execution_duration_s=self.duration, control_hz=1/self.dt,
                    maximum_interval_velocity_rad_s=np.abs(self.segment_velocity).max(0).tolist(),
                    initial_q_rad=self.initial.tolist(), joint_names=list(self.names),
                    calibration_reference=dict(workcell_fingerprint=self.metadata['workcell']['fingerprint'],
...
async def stream(trajectory, backend, output, *, start_tolerance_rad, tracking_tolerance_rad,
                 maximum_feedback_age_s, maximum_lateness_s, hold_s=1.,
    ...
    if (not np.isfinite([maximum_feedback_age_s, maximum_lateness_s, hold_s]).all() or
            not 0 < maximum_lateness_s < trajectory.dt or maximum_feedback_age_s <= 0 or hold_s < 0):
    ...
    try:
        ...
        with (output/'commands.jsonl').open('w') as log:
            for i in range(len(trajectory.times)+hold_count):
                ...
                velocity = trajectory.data['q_arm_velocity_rad_s'][index] if i < len(trajectory.times) else np.zeros(6)
            ...
            await sleep(max(0., start+(len(trajectory.times)+hold_count)*trajectory.dt-clock()))
...
class MockBackend:
    ...
    async def send(self, q, arm_velocity, dt): self.q = np.asarray(q).copy()
