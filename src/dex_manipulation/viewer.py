...
def viewer_payload(trajectory_path, report_path=None, model=None, object_mesh_path=None, reference_dir=None):
    if model is None:
    ...
    joints = []
    for j in raw["joints"]:
        joints.append({k: j[k] for k in ("name", "parent", "child", "kind", "frame0", "frame1", "reversed") if k in j}
                      | {"axis": j.get("axis", [0, 0, 0])})
    all_points = np.concatenate([d[k].reshape(-1, 3) for k in ("human_keypoints", "robot_keypoints", "object_keypoints")])
    all_points = all_points[np.isfinite(all_points).all(1)]
    geometry = []
    for collider in model.colliders:
        geometry.append(dict(link=collider["link"], vertices=numbers(collider["vertices"]), faces=collider["faces"]))
    object_mesh = None
    if object_mesh_path is not None:
        metadata_path=Path(object_mesh_path).with_suffix('.json')
        if metadata_path.exists():
            object_geometry=json.loads(metadata_path.read_text())
            if object_geometry.get('fingerprint') and str(d.get('object_geometry_fingerprint','')) != object_geometry['fingerprint']:
    ...
    for human, robot in zip(d["human_keypoints"], d["robot_keypoints"]):
        if not np.isfinite(human).all() or not np.isfinite(robot).all():
            ...
            quality.append(dict(keypoint_rmse_mm=float(np.sqrt(np.mean(np.sum((human-robot)**2, axis=1)))*1000),
    ...
    if reference_dir is not None:
        ...
        for frame in d["frame_ids"]:
            photos.append("data:image/jpeg;base64," + base64.b64encode(
                (reference_dir / f"color_{frame:06d}.jpg").read_bytes()).decode("ascii"))
        ...
        fx, cx = np.linalg.lstsq(np.c_[xyz[:, 0]/xyz[:, 2], np.ones(len(xyz))], uv[:, 0], rcond=None)[0]
        fy, cy = np.linalg.lstsq(np.c_[xyz[:, 1]/xyz[:, 2], np.ones(len(xyz))], uv[:, 1], rcond=None)[0]
        ...
        reference = dict(images=photos, points_2d=numbers(raw_uv), width=image_shape[1], height=image_shape[0],
                         intrinsics=numbers([fx, fy, cx, cy]), reprojection_max_px=error)
    return dict(frame_ids=d["frame_ids"].tolist(), times=numbers(times), timed=timed, display_frame=display_frame,
                ...
                valid=d["valid"].tolist(), transition_valid=d["transition_valid"].tolist(),
                human=numbers(d["human_keypoints"]), robot=numbers(d["robot_keypoints"]),
                wrist=numbers(d["wrist_transform"]), q=numbers(d["active_q_rad"]),
                object_poses=numbers(d["object_transform"]), object_points=numbers(d["object_points_local"]),
                object_mesh=object_mesh, geometry=geometry, reference=reference, quality=quality,
                model=dict(root=raw["root_link"], joints=joints, full_names=model.full_names,
                           coupling=numbers(model.coupling), offset=numbers(model.offset), keypoints=model.keypoints),
...
def comparison_viewer(trajectory_path, output_path, report_path=None, model=None, object_mesh_path=None, reference_dir=None):
    ...
    document = template.replace("__PAYLOAD__", json.dumps(payload, separators=(",", ":"), ensure_ascii=False).replace("</", "<\\/"))
