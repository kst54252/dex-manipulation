...
def run(env, metadata, checkpoint, output, *, replay_path=None, hold_s=1.):
    import torch
    ...
    if replay_path is None:
        ...
        learner.actor.eval(); learner.critic.eval()
    env.set_training(False); env.evaluation_protocol = 'strict'; env.reset(randomize=False)
    ...
    if frozen:
        if frozen.names != tuple(names) or frozen.metadata['checkpoint_sha256'] != sha256(checkpoint):
            raise ValueError('Recorded names/checkpoint changed')
        if (not np.allclose(frozen.metadata['world_from_source'], env.world_from_source, atol=1e-10) or
                frozen.metadata['workcell'] != env.workcell.metadata() or
                frozen.metadata['arm_asset_sha256'] != env.metadata['assembled_usd_sha256'] or
                frozen.metadata['arm_model_sha256'] != env.metadata['arm_model_sha256'] or
                not np.isclose(frozen.dt, env.task.dt)):
        ...
        for ids, q in ((env.arm_ids, qa), (env.full_ids, torch.tensor(qf[None], device=env.device, dtype=torch.float32))):
    ...
    with torch.no_grad():
        for i in range(count+hold_count):
            ...
            if not frozen and not hold:
                ...
                target_q = np.r_[info['applied_targets']['q_arm'][0].cpu().numpy(),
                ...
                good_ik = metrics['arm_ik_success'] > .5
                if not good_ik or metrics['early_failure'] or (bool(term[0] or trunc[0]) and i != count-1):
                ...
                tensor = lambda value: torch.tensor(np.asarray(value)[None], device=env.device, dtype=torch.float32)
                env.robot.set_joint_position_targets(tensor(target_q[:6]), joint_indices=env.arm_ids)
                ...
                metrics = dict(grasp_opposition_duty=float((contacts[:, 0]&contacts[:, 1:].any(1)).mean()),
                               grasp_all_five_duty=float(contacts.all(1).mean()),
                               grasp_contact_count=float(contacts.sum(1).mean()))
                good_ik = True
                if env.render: env.world.render()
            ...
            expected = env.reference.points(reference['object_position'], reference['object_quaternion'])[0]
            actual = env.reference.points(state['object_position'][None], state['object_quaternion'][None])[0]
            ...
            row = dict(timestamp_s=(i+1)*env.task.dt, hold=hold,
                       q_arm=state['q_arm'], q_finger=state['q'], q_full=state['full_q'],
                       object_position_source=state['object_position'], object_quaternion_xyzw=state['object_quaternion'],
                       object_error_m=np.linalg.norm(actual-expected, axis=1).mean(),
                       wrist_actual_base=position, wrist_command_base=target_pose,
                       wrist_position_error_m=np.linalg.norm(error[:3]), wrist_rotation_error_rad=np.linalg.norm(error[3:]),
                       coupling_error_rad=np.abs(state['full_q']-state['q']@env.model.coupling.T-env.model.offset).max(),
                       sigma_min=sigma, condition=condition,
                       opposition_duty=metrics['grasp_opposition_duty'],
                       all_five_duty=metrics['grasp_all_five_duty'], contact_count=metrics['grasp_contact_count'])
            if not all(np.isfinite(v).all() for v in row.values()):
                raise RuntimeError('Nonfinite recorded physics')
            rows.append(row)
            if not hold:
                commands.append(target_q.copy()); velocities.append(velocity.copy()); ik_ok.append(good_ik)
    ...
    motion = ~arrays['hold']; held = arrays['hold']
    lift = arrays['object_position_source'][:, 2]-initial_object[2]
    # A lift and a held opposing-finger contact, not completion alone.
    grasp = bool(lift[held].min() > .05 and arrays['opposition_duty'][held].min() >= .75
                 and arrays['object_error_m'].max() < env.cfg['success_object_error_m'])
    report = dict(schema='recorded_grasp_report_v1', simulated_grasp=grasp,
                  mode='frozen_joint_commands' if frozen else 'policy_capture',
                  command_count=count, control_dt_s=env.task.dt, reference_duration_s=env.reference.duration,
                  execution_duration_s=count*env.task.dt, hold_s=hold_count*env.task.dt,
                  checkpoint_sha256=sha256(checkpoint), checkpoint_iteration=saved['iteration'],
                  object_estimator_required_for_replay=False, policy_calls=0 if frozen else count,
                  online_ik_calls=0 if frozen else count, object_writes_during_motion=0,
                  ...
                  final_lift_mm=float(lift[-1]*1000), minimum_hold_lift_mm=float(lift[held].min()*1000),
                  ...
                  strict_coupling_0_01rad_pass=bool(arrays['coupling_error_rad'].max()<.01),
                  hardware_validated=False)
    if not frozen:
        m = dict(schema=SCHEMA, angle_unit='rad', length_unit='m', quaternion_order='xyzw',
                 command_semantics='start_of_interval_zero_order_hold', control_dt_s=env.task.dt,
                 reference_duration_s=env.reference.duration, execution_duration_s=count*env.task.dt,
                 ...
                 policy_contract_hash=metadata['contract_hash'], config=metadata['config'],
                 world_from_source=env.world_from_source.tolist(), workcell=env.workcell.metadata(),
                 ...
                 capture_report=report, hardware_calibrated=False)
        path = output/'commands.npz'
        np.savez_compressed(path, timestamp_s=np.arange(count)*env.task.dt,
                            q_command_rad=np.asarray(commands), q_arm_velocity_rad_s=np.asarray(velocities),
                            initial_q_rad=initial, joint_names=np.asarray(names), ik_success=np.asarray(ik_ok),
                            lower_rad=np.r_[env.arm.lower, env.model.lower], upper_rad=np.r_[env.arm.upper, env.model.upper],
                            velocity_limit_rad_s=np.r_[env.arm.velocity, env.model.velocity], metadata_json=np.asarray(json.dumps(m)))
    ...
    if not grasp:
        raise RuntimeError('Saved diagnostics, but this run did not pass the simulated lift-and-hold criterion')
    return report
