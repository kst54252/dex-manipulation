...
def stepped_can(spec):
    ...
    if (spec['length_unit'] != 'm' or spec['axis'] != '+Z' or center.shape != (3,)
            or not np.isfinite([r, R, h, b, spec['mass_kg'], *center]).all()
            or not 0 < r <= R or not 0 < b < h or spec['mass_kg'] <= 0
            or not isinstance(n, int) or isinstance(n, bool) or n < 16):
        raise ValueError('Invalid stepped can dimensions/frame')
    z0 = -h/2
    angles = np.arange(n)*2*np.pi/n
    rings = [(R, z0), (R, z0+b), (r, z0+b), (r, h/2)]
    vertices = np.vstack([np.c_[radius*np.cos(angles), radius*np.sin(angles), np.full(n, z)]
                          for radius, z in rings] + [np.array([[0, 0, z0], [0, 0, h/2]])]) + center
    faces = []
    for ring in range(3):
        for j in range(n):
            k = (j+1) % n
    ...
    for name, radius, height, z in [('Base', R, b, z0+b/2), ('Body', r, h-b, b/2)]:
        local = np.eye(4); local[:3, 3] = center + [0, 0, z]
        shapes.append(dict(name=name, type='cylinder', radius=radius, height=height, transform=local.tolist()))
...
def build_can(spec_path, asset_path, model_dir):
    ...
    mesh.CreateDisplayColorAttr([Gf.Vec3f(.12, .55, .59)])
    for shape in shapes:
        cylinder = UsdGeom.Cylinder.Define(stage, '/Can/Collision'+shape['name'])
        cylinder.CreateAxisAttr('Z'); cylinder.CreateRadiusAttr(shape['radius'])
    ...
    metadata.update(name=spec['name'], specification=spec,
                    frame='dataset object pose frame; original geometric center and +Z axis preserved',
                    center_of_mass_m=com.tolist(), diagonal_inertia_kg_m2=inertia.tolist(),
    ...
    points = generate_object_points(model_dir/'can_mesh.npz', model_dir/'can_points.npz',
                                    count=spec['surface_point_count'], seed=spec['surface_sampling_seed'])
    overlay = Usd.Stage.CreateNew(str(asset_path.with_name('keypoints.usda')))
    UsdGeom.SetStageMetersPerUnit(overlay, 1.); UsdGeom.SetStageUpAxis(overlay, 'Z')
    ...
    markers.CreateWidthsAttr([.0015]*len(points))
    markers.CreateIdsAttr(list(range(len(points))))
    markers.CreateDisplayColorAttr([Gf.Vec3f(1., .75, .05)])
    overlay.GetRootLayer().Save()
    np.savetxt(asset_path.with_name('keypoints.csv'), np.c_[np.arange(len(points)), points], delimiter=',',
               header='index,x_m,y_m,z_m', comments='', fmt=['%d','%.12g','%.12g','%.12g'])
    return metadata
