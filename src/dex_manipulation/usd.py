...
def extract_hand(stage_path, keypoint_path, output_path):
    ...
    raw_keypoints = json.loads(Path(keypoint_path).read_text())
    ...
    for prim in prims:
        ...
        if len(a) != 1 or len(b) != 1 or not a[0].HasPrefix(root_path) or not b[0].HasPrefix(root_path):
            continue
        kind = {"PhysicsRevoluteJoint": "revolute", "PhysicsPrismaticJoint": "prismatic",
                "PhysicsFixedJoint": "fixed"}.get(prim.GetTypeName())
        if kind is None:
            raise ValueError(f"Unsupported joint: {prim.GetPath()} {prim.GetTypeName()}")
        record = dict(name=prim.GetName(), usd_path=str(prim.GetPath()), body0=str(a[0]), body1=str(b[0]),
                      kind=kind, frame0=frame(joint, 0), frame1=frame(joint, 1), mimic=None)
        if kind != "fixed":
            factor = np.pi / 180 if kind == "revolute" else scale
            record["axis"] = np.eye(3)["XYZ".index(prim.GetAttribute("physics:axis").Get())].tolist()
            ...
            record.update(velocity=velocity, velocity_source="physxJoint:maxJointVelocity" if physx_v is not None else "urdf:limit:velocity")
            record["legacy_urdf_velocity"] = None if urdf_v is None else float(urdf_v)
            record["velocity_overrides_urdf"] = bool(urdf_v is not None and not np.isclose(velocity, urdf_v, rtol=1e-4))
            ...
            if leaders and prim.GetAttribute("newton:mimicEnabled").Get() is not False:
                ...
                record["mimic"] = dict(leader=leaders[0].name, multiplier=1.0 if coef1 is None else float(coef1),
                                       offset=0.0 if coef0 is None else float(coef0 * factor), schema="NewtonMimicAPI")
            unsupported = [a.GetName() for a in prim.GetAttributes() if "mimic" in a.GetName().lower() and not a.GetName().startswith("newton:")]
            if unsupported:
    ...
    while remaining:
        ...
        for joint in remaining[:]:
            ...
            if (a in reached) != (b in reached):
                ...
                joint.update(parent=parent.split("/")[-1], child=child.split("/")[-1], reversed=reverse)
                reached.add(child)
    ...
    zero_links = {name: (root_inverse @ world(stage.GetPrimAtPath(path))).tolist() for name, path in link_paths.items()}
    ...
    for k in raw_keypoints:
        ...
        matches = [p for p in prims if p.GetName() == k["name"] and p.GetParent().GetPath() == link_paths[k["parent_link"]]]
        if len(matches) != 1:
            raise ValueError(f"Missing or ambiguous USD keypoint: {k['name']}")
        local = inverse(world(stage.GetPrimAtPath(link_paths[k["parent_link"]]))) @ world(matches[0])
        if not np.allclose(local[:3, 3], k["xyz"], atol=1e-7):
            raise ValueError(f"JSON local coordinates disagree with USD: {k['name']}")
        keypoints.append(dict(name=k["name"], semantic=re.sub(r"^kp_\d+_", "", k["name"]),
                              link=k["parent_link"], xyz=k["xyz"]))
    ...
    for prim in prims:
        ...
        colliders.append(dict(link=parent.GetName(), usd_path=str(prim.GetPath()), approximation=approximation,
                              vertices=vertices.tolist(), faces=faces.tolist(), original_vertex_count=len(points)))
    if not colliders:
        raise ValueError("No collision geometry extracted")
    description = dict(schema_version=1, source=str(Path(stage_path).resolve()), source_sha256=file_hash(stage_path),
                       keypoint_sha256=file_hash(keypoint_path), length_unit="m", angle_unit="rad", quaternion_order="xyzw",
                       usd_meters_per_unit=scale, usd_up_axis=str(UsdGeom.GetStageUpAxis(stage)), root_link=root_name,
                       root_usd_path=str(root_path), link_paths=link_paths, joints=ordered, keypoints=keypoints,
                       colliders=colliders, authored_zero_link_transforms=zero_links)
...
def extract_object(mesh_stage_path, collision_stage_path, output_path):
    ...
    cache = UsdGeom.XformCache()
    root_inverse = np.linalg.inv(np.array(cache.GetLocalToWorldTransform(collision_stage.GetDefaultPrim())).T)
    shapes = []
    for prim in collision_stage.Traverse():
        ...
        axis_rotation = {"X": Rotation.from_rotvec([0, np.pi / 2, 0]).as_matrix(),
                         "Y": Rotation.from_rotvec([-np.pi / 2, 0, 0]).as_matrix(), "Z": np.eye(3)}[axis]
        local = local @ transform(axis_rotation)
        shapes.append(dict(type="cylinder", radius=cylinder.GetRadiusAttr().Get() * collision_scale,
                           height=cylinder.GetHeightAttr().Get() * collision_scale, transform=local.tolist(),
                           usd_path=str(prim.GetPath())))
    if not shapes:
        raise ValueError("No object collision geometry")
    output_path = Path(output_path)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    np.savez_compressed(output_path, vertices=np.asarray(vertices), faces=np.asarray(faces, dtype=np.int32))
    metadata = dict(mesh_source=str(mesh_stage_path), collision_source=str(collision_stage_path),
                    units="m", frame="mesh stage default prim; alignment with dataset must be confirmed",
                    stage_up_axis=str(UsdGeom.GetStageUpAxis(stage)), collision_shapes=shapes,
                    bounds_m=[np.min(vertices, axis=0).tolist(), np.max(vertices, axis=0).tolist()])
    import hashlib
    from .geometry import geometry_fingerprint
    metadata.update(mesh_sha256=hashlib.sha256(output_path.read_bytes()).hexdigest(),
...
def extract_arm(stage_path, base_path, flange_path, wrist_path, output_path):
    ...
    for prim in stage.Traverse():
        ...
        if paths[0] == paths[1]:
            continue
        record = dict(name=prim.GetName(), usd_path=str(prim.GetPath()), mimic=None,
                      attachment_paths=[str(t[0]) for t in targets], body0=paths[0], body1=paths[1])
        for i in (0, 1):
            quat = getattr(joint, f"GetLocalRot{i}Attr")().Get()
            local = transform(Rotation.from_quat([*quat.GetImaginary(), quat.GetReal()]).as_matrix(),
    ...
    queue = deque([(base_path, [])]); seen = {base_path}; chain = None
    while queue:
        ...
        for child, record, reverse in adjacency.get(parent, []):
            if child in seen:
                continue
            seen.add(child)
            queue.append((child, prefix + [dict(record, parent=parent.split('/')[-1],
                                               child=child.split('/')[-1], reversed=reverse)]))
    if not chain:
    ...
    after_flange = False; mount = np.eye(4)
    for j in chain:
    ...
    observed_mount = inverse(world(stage.GetPrimAtPath(flange_path))) @ world(stage.GetPrimAtPath(wrist_path))
    ...
    description = dict(schema_version=1, source=str(Path(stage_path).resolve()),
                       source_sha256=file_hash(stage_path),
                       layer_sha256={str(Path(layer.realPath).resolve()): file_hash(layer.realPath)
                                     for layer in stage.GetUsedLayers() if layer.realPath},
                       length_unit='m', angle_unit='rad', quaternion_order='xyzw',
                       usd_meters_per_unit=scale, usd_up_axis=str(UsdGeom.GetStageUpAxis(stage)),
                       root_link=base_path.split('/')[-1], root_usd_path=base_path,
                       flange_link=flange_name, flange_usd_path=flange_path,
                       wrist_link=wrist_path.split('/')[-1], wrist_usd_path=wrist_path,
                       flange_frame_definition='selected USD last arm rigid-link frame; no inferred flange face/TCP',
                       flange_to_wrist=mount.tolist(), joints=chain, keypoints=[], colliders=[],
                       link_paths=link_paths, base_in_stage=world(stage.GetPrimAtPath(base_path)).tolist(),
                       authored_zero_link_transforms={name:(root_inv @ world(stage.GetPrimAtPath(path))).tolist()
                                                      for name,path in link_paths.items()})
    from .fk import ArmModel
    model = ArmModel(description)
    fk = model.link_transforms(np.zeros(6))
    zero_error = max(np.max(np.abs(fk[name] - pose)) for name,pose in description['authored_zero_link_transforms'].items())
    if zero_error > 2e-6:
