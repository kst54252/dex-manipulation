...
class FloatingSurface:
    ...
    def __init__(self, description, initial_object_position):
        ...
        if (size.shape != (2,) or center.shape != (2,) or initial.shape != (3,) or
                not np.isfinite(np.r_[size, center, initial, top, thickness]).all() or
                np.any(size <= 0) or thickness <= 0 or top != 0):
        ...
        self.translation = np.r_[center-initial[:2], 0.]
        self.fingerprint = hashlib.sha256(json.dumps(description, sort_keys=True).encode()).hexdigest()

    def boxes(self):
        d = self.description
        return {'Table': dict(size=[*d['size_xy_m'], d['thickness_m']],
                              center=[*d['center_xy_m'], d['top_z_m']-d['thickness_m']/2])}

    def metadata(self):
        return dict(kind='finite_plane', fingerprint=self.fingerprint,
                    tabletop_z_m=self.description['top_z_m'], lower_floor=False,
                    floating_source_translation_m=self.translation.tolist(), geometry=self.boxes())

    def create_usd(self, stage, prefix, origin=(0.,0.,0.)):
        ...
        cube.CreateDisplayColorAttr([(.24,.29,.34)])
...
class Workcell:
    def __init__(self, description):
        ...
        if not np.isclose(table['top_z'], 0., atol=1e-12):
            raise ValueError('Tabletop must define world Z=0')
        if (not np.isclose(base['center'][2]+base['size'][2]/2, base['top_z']) or
                not np.isclose(base['center'][2]-base['size'][2]/2, d['floor_z']) or
                not np.isclose(d['robot_mount']['position'][2], base['top_z'])):
            raise ValueError('Pedestal, floor and robot mount heights disagree')
        mount = d['robot_mount']
        self.world_from_base = transform(Rotation.from_quat(mount['quaternion_xyzw']).as_matrix(), mount['position'])
        self.floating_translation = np.asarray(d['floating_source_translation_m'], float)
        if self.floating_translation.shape != (3,) or not np.isfinite(self.floating_translation).all() or self.floating_translation[2] != 0:
            raise ValueError('Floating placement must be a finite XY translation; preserve tabletop Z=0')
        for box in self.boxes().values():
            if not np.isfinite([box['size'], box['center']]).all() or np.any(np.asarray(box['size']) <= 0):
    ...
    def boxes(self):
        ...
        result = {
            'Table': dict(size=[*table['size_xy'], thickness], center=[*table['center_xy'], top-thickness/2]),
            'Pedestal': dict(size=base['size'], center=base['center']),
            'Floor': dict(size=[*floor['size_xy'], floor['thickness']], center=[*floor['center_xy'], d['floor_z']-floor['thickness']/2]),
        }
        height = top-thickness-d['floor_z']
        for i, xy in enumerate(table['leg_centers_xy']):
            result[f'Leg{i+1}'] = dict(size=[*table['leg_size_xy'], height], center=[*xy, d['floor_z']+height/2])
        return result

    def metadata(self):
        return dict(fingerprint=self.fingerprint, world_from_base=self.world_from_base.tolist(),
                    tabletop_z_m=self.description['table']['top_z'], floor_z_m=self.description['floor_z'],
                    floating_source_translation_m=self.floating_translation.tolist(), geometry=self.boxes())
    ...
    def resolve_alignment(self, alignment):
        ...
        if 'world_from_source' in result:
            ...
            if 'base_from_source' in result and not np.allclose(base, result['base_from_source'], atol=1e-10, rtol=0):
        ...
        if result['status'].startswith('simulation_ground') and not np.allclose(world[2], [0,0,1,0], atol=1e-9, rtol=0):
        ...
        result['scene_placement'] = dict(workcell_fingerprint=self.fingerprint,
                                        world_from_base=self.world_from_base.tolist(), world_from_source=world.tolist())
        return result

    def validate_reference(self, metadata, alignment):
        resolved = self.resolve_alignment(alignment)
        if (metadata.get('scene_placement') != resolved['scene_placement'] or
                not np.allclose(metadata['base_from_source'], resolved['base_from_source'], atol=1e-10, rtol=0)):
            raise ValueError('Arm reference predates this workcell/placement. Run: python scripts/ik.py solve')

    def create_usd(self, stage, prefix='/Workcell', origin=(0.,0.,0.), collision=True):
        """Seven boxes, same visible and collision geometry; no physics tuning."""
        from pxr import UsdGeom, UsdPhysics, Gf
        UsdGeom.Xform.Define(stage, prefix)
        colors = {'Table': (.36,.25,.15), 'Pedestal': (.19,.23,.28), 'Floor': (.10,.12,.15)}
        result = {}
        for name, box in self.boxes().items():
            ...
            cube.AddTranslateOp().Set(Gf.Vec3d(*(np.asarray(box['center'])+origin)))
            cube.AddScaleOp().Set(Gf.Vec3d(*box['size']))
            cube.CreateDisplayColorAttr([colors.get(name, (.22,.24,.27))])
            if collision:
                UsdPhysics.CollisionAPI.Apply(cube.GetPrim())
