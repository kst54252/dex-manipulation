...
def prepare_arm_articulation(stage, assembly_path, base_path, wrist_path):
    ...
    if len(ancestors) != 1:
        raise ValueError(f'Expected one articulation ancestor of {base_path}; found {[str(p.GetPath()) for p in ancestors]}')
    chosen = ancestors[0]
    ...
    for prim in prims:
        ...
        if joint.GetJointEnabledAttr().Get() is False or joint.GetExcludeFromArticulationAttr().Get():
            continue
        a, b = rigid_owner(joint.GetBody0Rel().GetTargets()), rigid_owner(joint.GetBody1Rel().GetTargets())
    ...
    while pending:
        ...
        if body not in connected:
            connected.add(body); pending.extend(graph[body] - connected)
    if not anchored or str(wrist.GetPath()) not in connected:
        raise ValueError('Expected a world-fixed arm base with an enabled joint path to the mounted hand')
    removed = []
    ...
    for prim in roots:
        ...
        removed.append(prim)
    for prim in removed:
        for schema in ('NewtonArticulationRootAPI', 'PhysicsArticulationRootAPI', 'PhysxArticulationAPI'):
    ...
    return chosen, dict(root=str(chosen.GetPath()), roots_before=[str(p.GetPath()) for p in roots],
                        removed_connected_roots=[str(p.GetPath()) for p in removed],
                        connected_rigid_bodies=len(connected), fixed_base=anchored)
...
def replay_floating(root, output, loops=3, render=True, *, observer=None,
    ...
    if loops < 0:raise ValueError('loops must be nonnegative; 0 repeats until stopped')
    root, output = Path(root), Path(output)
    output.mkdir(parents=True, exist_ok=True)
    cfg=json.loads(Path(config_path or root/'config/policy.json').read_text())
    ...
    cfg['reference']=json.loads(Path(arm_config_path or root/'config/ik.json').read_text())['input']
        ...
        for loop in (range(loops) if loops else count()):
            ...
            for step in range(int(np.ceil(reference.duration/env.task.dt))+1):
                ...
                if render:
                    ...
                    if delay>0:time.sleep(min(delay,env.task.dt))
            failures.append(first_failure)
            physics_steps.append(elapsed_physics_steps)
        if not rows:
            (output/'report.json').write_text(json.dumps(dict(robot='floating',samples=0,interrupted=True))+'\n')
            return
