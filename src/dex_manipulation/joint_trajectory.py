...
class JointReference:
    def __init__(self, path):
        ...
        if (
            len(self.names) != 12
            or len(set(self.names)) != 12
            or self.positions.shape != (len(self.times), 12)
        ):
        ...
        if np.any(d["joint_velocity_limits_rad_s"] <= 0) or np.any(
            d["joint_lower_rad"] >= d["joint_upper_rad"]
        ):
            raise ValueError("Invalid joint limit intervals")
        if np.any(self.positions < d["joint_lower_rad"] - 1e-7) or np.any(
            self.positions > d["joint_upper_rad"] + 1e-7
        ):
    ...
    def iter_targets(self, rate_hz=None):
        if rate_hz is None:
            ...
            times = (
                self.times[0]
                + np.arange(int(np.ceil((self.times[-1] - self.times[0]) * rate_hz))) / rate_hz
            )
    ...
    def export_csv(self, path, rate_hz=None):
        ...
        np.savetxt(
            ...
            rows,
            delimiter=",",
            header=",".join(["timestamp_s", *[n + "_rad" for n in self.names]]),
            comments="",
        )
