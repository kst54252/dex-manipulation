...
def resolve_demo_path(path, root=None):
    """Resolve historical input paths without rewriting immutable NPZ provenance.
...
def legacy_demo_paths(value):
    """Stable checkpoint-hash spelling for the two renamed data directories.
...
def import_human_demo(source, output, geometry_path):
    ...
    with np.load(source,allow_pickle=False) as data:
        values={k:data[k].copy() for k in (
            'source_frame_indices','mano_joint_coords_right_mano21','mano_joint_coords_original',
            'object_pos','object_quat','quaternion_order','fps','sequence_name','source_camera_serial',
            'source_hand_side','target_hand_side','hand_conversion','mano_source_joint_order',
            'source_grasped_ycb_id','grasped_object_name')}
    ids=values['source_frame_indices'];frame_mask(ids)
    ...
    position=np.asarray(values['object_pos'],dtype=float);quaternion=np.asarray(values['object_quat'],dtype=float)
    if (hand.shape!=(len(ids),21,3) or original.shape!=hand.shape or position.shape!=(len(ids),3)
            or quaternion.shape!=(len(ids),4) or not all(np.isfinite(v).all() for v in (hand,original,position,quaternion))):
        raise ValueError('Invalid demo landmark/object shapes or nonfinite annotations')
    if (str(values['mano_source_joint_order'])!='mano21_sequential_thumb_index_middle_ring_little'
    ...
    if order not in ('wxyz','xyzw') or not np.allclose(np.linalg.norm(quaternion,axis=-1),1,atol=1e-5):
        raise ValueError('Invalid declared quaternion order or norm')
    rotation=Rotation.from_quat(quaternion[:,[1,2,3,0]] if order=='wxyz' else quaternion).as_matrix()
    ...
    if str(values['source_hand_side'])=='left' and conversion=='object_local_x_reflection':
        ...
        expected=original
    else:raise ValueError('Unsupported or inconsistent hand-conversion metadata')
    reflection_error=float(np.max(np.abs(expected-hand)))
    if reflection_error>2e-6:raise ValueError('Right-hand annotations disagree with the declared conversion')
    fps=float(values['fps'])
    if not np.isfinite(fps) or fps<=0:raise ValueError('Demo must declare positive finite playback fps')
    ...
    world_from_camera[:3,:3]=[[1.,0.,0.],[0.,0.,1.],[0.,-1.,0.]]
    first=np.eye(4);first[:3]=poses[0,0]
    alignment,orientation=can_base_down_alignment(world_from_camera@first,geometry)
    metadata=dict(object_index=0,object_name=geometry.get('name',str(values['grasped_object_name'])),
        mesh_to_object=alignment.tolist(),hand_side='right',left_hand_policy='preserve_source',
        length_unit='m',camera_axes='x_right_y_down_z_forward',fps=None,
        grounding_mode='camera_negative_y_up',
        object_orientation_requirement='initial_base_below_body',object_alignment_check=orientation,
        timestamps_s=((ids-ids[0])/fps).tolist(),time_basis='source_declared_playback_fps',
        source_playback_fps=fps,frame_range=[int(ids[0]),int(ids[-1])],
        dataset_object_name=str(values['grasped_object_name']),source_grasped_ycb_id=int(values['source_grasped_ycb_id']),
        object_geometry=str(geometry_path),
        source_demo=dict(path=str(source.resolve()),sha256=hashlib.sha256(source.read_bytes()).hexdigest(),
            sequence=str(values['sequence_name']),camera=str(values['source_camera_serial']),
            source_hand_side=str(values['source_hand_side']),hand_conversion=conversion,
            conversion_max_error_m=reflection_error,source_quaternion_order=order),
    ...
    np.savez_compressed(paths[0],frame_ids=ids,pose_y=poses,joint_3d=hand[:,None],
