...
def checked_pose(value):
    ...
    if (
        not np.allclose(pose[3], [0, 0, 0, 1], atol=1e-9)
...
class IKOptions:
    ...
    jacobian_length_m: float = 0.5
    singular_sigma_min: float = 0.001
    near_sigma_min: float = 0.03
    max_condition: float = 10000.0
    near_condition: float = 200.0
    damping_min: float = 1e-6
    damping_max: float = 0.04
    limit_warning_rad: float = np.deg2rad(5.0)
    limit_margin_rad: float = 1e-4
    max_joint_step_rad: float = np.deg2rad(20.0)
    iteration_step_rad: float = 0.2
...
class ArmIK:
    def __init__(self, model: ArmModel, options=None):
        ...
        self.scale = np.array([1 / self.options.jacobian_length_m] * 3 + [1.0] * 3)
    ...
    def _result(self, q, target, iterations, damping, bounds):
        ...
        reasons = []
        if (
            pos > self.options.position_tolerance_m
            or angle > self.options.orientation_tolerance_rad
        ):
        ...
        return IKResult(
            ...
            margin,
            margin < self.options.limit_warning_rad,
            not reasons,
    ...
    def _solve_seed(self, target, seed, preferred, lower, upper):
        ...
        for iteration in range(o.max_iterations):
            ...
            if (
                np.linalg.norm(wrist_error[:3]) <= o.position_tolerance_m * 0.1
                and np.linalg.norm(wrist_error[3:]) <= o.orientation_tolerance_rad * 0.1
            ):
                break
            error = self.scale * pose_error(flange, self.model.pose(q, "flange"))
            jac = self.scale[:, None] * self.model.jacobian(q, "flange")
            sigma = np.linalg.svd(jac, compute_uv=False)[-1]
            damping = o.damping_min + o.damping_max * max(0.0, 1 - sigma / o.near_sigma_min) ** 2
            ...
            weight = np.sqrt(1 + np.minimum((o.limit_warning_rad / distance) ** 2, 100.0))
            center = (self.model.lower + self.model.upper) / 2
            bias = np.clip(
                0.05 * (center - q) / (self.model.upper - self.model.lower) + 0.1 * (preferred - q),
                -0.1,
                0.1,
            )
            penalty = damping * np.diag(weight)
            step = np.linalg.lstsq(
                np.vstack([jac, penalty]), np.r_[error, penalty @ bias], rcond=None
            )[0]
    ...
    def solve(self, wrist_target, seed, previous=None, dt=None):
        ...
        for i in range(count - 1):
            ...
            if previous is None and i >= 5:
                ...
                radius = 0.35 if previous is None else 0.12
        ...
        if valid:
            ...
            result = min(
                valid, key=lambda r: (np.linalg.norm(r.q - preferred), -r.joint_limit_distance_rad)
            )
        else:
            result = min(
                candidates,
                key=lambda r: (
                    r.position_error_m / self.options.jacobian_length_m + r.orientation_error_rad
                ),
            )
    ...
    def transition(self, before, after):
        ...
        return dict(
            sigma_min=float(values[:, 0].min()),
            condition_max=float(values[:, 1].max()),
            valid=bool(
                values[:, 0].min() >= self.options.singular_sigma_min
                and values[:, 1].max() <= self.options.max_condition
            ),
        )
...
def interpolate_ik_source(source, count, substeps=1):
    ...
    sample_times = (
        np.r_[
            np.concatenate(
                [np.linspace(a, b, substeps, endpoint=False) for a, b in zip(times[:-1], times[1:])]
            ),
            times[-1],
        ]
        if count > 1
    ...
    n = len(sample_times)
    originals = np.arange(count) * substeps
    mask = np.zeros(n, bool)
    mask[originals] = True
    frame_ids = np.full(n, -1, dtype=ids.dtype)
    frame_ids[originals] = ids
    intervals = np.stack((frame_ids, frame_ids), axis=1)
    valid = np.zeros(n, bool)
    transitions = np.zeros(n, bool)
    ...
    for i in range(count - 1):
        ...
        valid[span] = (
            source["valid"][i] and source["valid"][i + 1] and source["transition_valid"][i + 1]
        )
    ...
    out.update(
        ...
        source_frame_mask=mask,
        source_interval_frame_ids=intervals,
    )
    for name in ("wrist_transform", "object_transform"):
        ...
        result[:, :3, 3] = np.stack(
            [np.interp(sample_times, times, poses[:, i, 3]) for i in range(3)], axis=1
        )
        if count > 1:
            result[:, :3, :3] = Slerp(times, Rotation.from_matrix(poses[:, :3, :3]))(
                sample_times
            ).as_matrix()
        result[originals] = poses  # Preserve the original matrices exactly.
        out[name] = result
    q = source["active_q_rad"][:count]
    out["active_q_rad"] = np.stack(
        [np.interp(sample_times, times, q[:, i]) for i in range(q.shape[1])], axis=1
    )
...
def solve_trajectory(
    ...
    seed,
    options=None,
    alignment_status="provided_unverified",
    max_frames=None,
    ...
    *,
    ...
    if source["wrist_transform"].shape != (len(times), 4, 4) or any(
        source[k].shape != (len(times),) for k in ("frame_ids", "valid", "transition_valid")
    ):
    ...
    for pose in source["wrist_transform"]:
        checked_pose(pose)
    if (
        not np.isfinite(finger).all()
        or np.any(finger < hand.lower - 1e-7)
        or np.any(finger > hand.upper + 1e-7)
    ):
    ...
    for i, target in enumerate(targets):
        result = solver.solve(
            target,
            seed if last_q is None else last_q,
            previous=last_q,
            dt=None if last_time is None else times[i] - last_time,
        )
        ...
        if i > 0:
            ...
            if last_index == i - 1 and result.success:
                ...
                if not transition["valid"]:
                    ...
                    result.reason += ";singular_transition"
        if not source["valid"][i] or (i > 0 and not source["transition_valid"][i]):
            result.success = False
            result.reason += ";invalid_source_reference"
        if i > 0 and np.any(
            np.abs(finger[i] - finger[i - 1]) / (times[i] - times[i - 1]) > hand.velocity + 1e-6
        ):
            result.success = False
            result.reason += ";finger_velocity_limit"
        ...
        label = (
            source["frame_ids"][i]
            if source["source_frame_mask"][i]
            else f"{source['source_interval_frame_ids'][i].tolist()} intermediate"
        )
        print(
            f"[ik] frame={label} valid={result.success} "
            f"pos={result.position_error_m:.3g}m rot={result.orientation_error_rad:.3g}rad "
            f"sigma={result.sigma_min:.4g} {result.reason}",
            flush=True,
        )
    ...
    metadata = dict(
        schema="dex_arm_hand_reference_v1",
        length_unit="m",
        angle_unit="rad",
        quaternion_order="xyzw",
        target_frame="RB3 USD base link",
        source_wrist_frame=hand.root,
        base_from_source=alignment.tolist(),
        alignment_status=alignment_status,
        hardware_calibrated=False,
        arm_model_source_sha256=model.description["source_sha256"],
        arm_fingerprint=model_fingerprint(model),
        ...
        initial_seed_q_rad=np.asarray(seed).tolist(),
        initial_robot_state_verified=False,
        model_joint_order=model.active_names,
        finger_joint_order=hand.active_names,
        ...
        trajectory_substeps=trajectory_substeps,
    ...
    arrays = dict(
        timestamps_s=times,
        frame_ids=source["frame_ids"][:n],
        q_arm=arm,
        q_finger=finger,
        source_frame_mask=source["source_frame_mask"],
        source_interval_frame_ids=source["source_interval_frame_ids"],
        joint_position_rad=np.c_[arm, finger],
        joint_names=np.array(model.active_names + hand.active_names),
        arm_joint_names=np.array(model.active_names),
        finger_joint_names=np.array(hand.active_names),
        joint_velocity_limits_rad_s=np.r_[model.velocity, hand.velocity],
        joint_lower_rad=np.r_[model.lower, hand.lower],
        joint_upper_rad=np.r_[model.upper, hand.upper],
        wrist_target_source=wrist_source,
        wrist_target_pose=targets,
        ...
        sigma_min=np.array([r.sigma_min for r in results]),
        condition=np.array([r.condition for r in results]),
        ...
        failure_reason=np.array([r.reason for r in results]),
    ...
    report = dict(
        ...
        position_error_m=dict(
            mean=float(arrays["position_error_m"].mean()),
            maximum=float(arrays["position_error_m"].max()),
        ),
        orientation_error_rad=dict(
            mean=float(arrays["orientation_error_rad"].mean()),
            maximum=float(arrays["orientation_error_rad"].max()),
        ),
        ...
        near_singular_transition_frame_ids=source["frame_ids"][:n][
            ...
            | (arrays["transition_condition_max"] > solver.options.near_condition)
        ].tolist(),
        ...
        failed_frame_indices=np.flatnonzero(~success).tolist(),
        failed_frame_ids=source["frame_ids"][:n][~success].tolist(),
