...
class MotionController:
    ...
    def __init__(self, model, count, dt, config, device='cpu'):
        ...
        coupled_v = torch.where(coeff > 1e-8, full_v[:, None]/coeff.clamp_min(1e-8), torch.inf).amin(0)
        self.joint_velocity_limit = torch.minimum(coupled_v, torch.as_tensor(model.velocity, dtype=torch.float32, device=device))
        if self.dt <= 0 or not math.isfinite(self.dt):
            raise ValueError('Motion controller dt must be positive and finite')
        if self.enabled:
            required = ('position_time_constant_s','rotation_time_constant_s','joint_time_constant_s',
                        'linear_velocity_m_s','linear_acceleration_m_s2',
                        'angular_velocity_rad_s','angular_acceleration_rad_s2','joint_acceleration_rad_s2')
            ...
            correction=config.get('contact_correction_velocity_m_s',5.)
        ...
        self.quaternion = torch.zeros(count, 4, device=device); self.quaternion[:, 3] = 1
    ...
    def reset(self, ids, state):
        ...
        self.velocity[ids] = 0; self.omega[ids] = 0; self.q_velocity[ids] = 0
    ...
    def _velocity(self, error, previous, maximum, acceleration, tau, vector):
        ...
        cap = torch.minimum(torch.as_tensor(maximum, device=error.device), (2*acceleration*distance).sqrt())
    ...
    def step(self, target):
        if not self.enabled:
            return target
        c = self.config
        self.velocity = self._velocity(target['position']-self.position, self.velocity,
            c['linear_velocity_m_s'], c['linear_acceleration_m_s2'], c['position_time_constant_s'], True)
        self.omega = self._velocity(rotation_error(target['quaternion'], self.quaternion), self.omega,
            c['angular_velocity_rad_s'], c['angular_acceleration_rad_s2'], c['rotation_time_constant_s'], True)
        self.position = self.position + self.velocity*self.dt
        self.quaternion = quat_normalize(quat_multiply(from_rotvec(self.omega*self.dt), self.quaternion))
        acceleration = c['joint_acceleration_rad_s2']
        self.q_velocity = self._velocity(target['active_q'].clamp(self.lower,self.upper)-self.q, self.q_velocity,
            self.joint_velocity_limit, acceleration, c['joint_time_constant_s'], False)
        ...
        def stopping_speed(distance):
            return ((acceleration*self.dt)**2 + 2*acceleration*distance.clamp_min(0)).sqrt()-acceleration*self.dt
        self.q_velocity = self.q_velocity.clamp(-stopping_speed(self.q-self.lower), stopping_speed(self.upper-self.q))
        self.q = self.q + self.q_velocity*self.dt
        return dict(target, position=self.position.clone(), quaternion=self.quaternion.clone(),
                    active_q=self.q.clone(), full_q=self.q@self.coupling.T+self.offset)
