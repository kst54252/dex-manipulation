...
class PolicyFrame:
    ...
    def __init__(self, world_from_source, device='cpu'):
        ...
        self.translation = torch.tensor(self.source_from_world[:3, 3], dtype=torch.float32, device=device)
        self.quaternion = torch.tensor(Rotation.from_matrix(self.source_from_world[:3, :3]).as_quat(),
...
class PolicyArmBridge:
    ...
    def target(self, position, quaternion):
        return self.base_from_source @ transform(Rotation.from_quat(quaternion).as_matrix(), position)
    ...
    def solve(self, position, quaternion, dt):
        ...
        transition = self.solver.transition(before, result.q) if result.success else dict(valid=False)
        valid = result.success and transition['valid']
        if valid:
            self.previous = result.q.copy()
        return dict(q_arm=self.previous.copy(), wrist_target=target, result=result,
                    success=valid, reason=result.reason if not result.success else 'ok' if valid else 'singular_transition',
                    transition=transition, joint_step_rad=np.abs(self.previous - before),
                    q_arm_velocity=(self.previous-before)/dt)


def named_material_transfer(source_names, source_counts, source_materials,
                            target_names, target_counts, target_materials):
    ...
    """
    source=np.asarray(source_materials);target=np.array(target_materials,copy=True)
    for names,counts,values in ((source_names,source_counts,source),(target_names,target_counts,target)):
    ...
    for name,count in zip(source_names,source_counts):
        if count:
            if name not in target_names:raise ValueError(f'Missing collider body: {name}')
            index=target_names.index(name)
            if count!=target_counts[index]:raise ValueError(f'Collider shape count changed: {name}')
            target[offsets[index]:offsets[index+1]]=source[source_start:source_start+count]
        source_start+=count
    return target
