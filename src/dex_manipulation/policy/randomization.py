...
def apply_startup_randomization(env):
    ...
    views = {
        "hand": env.robot._physics_view,
        "object": env.can._physics_view,
        "table": env.table._physics_view,
    }
    env.nominal_physics = {}
    for name, view in views.items():
        env.nominal_physics[name] = {
            "materials": view.get_material_properties().clone().cpu(),
            "masses": view.get_masses().clone().cpu(),
        }
    ...
    env.nominal_physics["hand"]["stiffness"] = (
        env.robot._physics_view.get_dof_stiffnesses().clone().cpu()
    )
    env.nominal_physics["hand"]["damping"] = (
        env.robot._physics_view.get_dof_dampings().clone().cpu()
    )
    if not cfg["enabled"]:
        env.randomized_physics = {
            name: {key: value.clone() for key, value in item.items()}
            for name, item in env.nominal_physics.items()
        }
    ...
    for name, view in views.items():
        ...
        if name == "hand" and cfg.get("hand_material_body_pattern"):
            ...
            selected = torch.tensor(
                [
                    bool(re.fullmatch(cfg["hand_material_body_pattern"], n))
                    for n, count in zip(env.robot.body_names, env.body_shape_counts)
                    for _ in range(count)
                ]
            )
            if len(selected) != materials.shape[1]:
                raise ValueError("Arm material shape mapping differs from PhysX")
            materials[:, ~selected] = nominal["materials"][:, ~selected]
        env.randomized_physics[name]["materials"] = materials
        if name != "table":
            mass = nominal["masses"]
            factor = torch.tensor(
                rng.uniform(*cfg[name + "_mass_scale"], mass.shape), dtype=mass.dtype
            )
            if name == "hand" and cfg.get("hand_mass_body_pattern"):
                ...
                selected = torch.tensor(
                    [
                        bool(re.fullmatch(cfg["hand_mass_body_pattern"], n))
                        for n in env.robot.body_names
                    ]
                )
                factor[:, ~selected] = 1.0
            ...
            env.randomized_physics[name]["inertias"] = inertia * factor.reshape(
                *inertia.shape[:-1], 1
            )
    ...
    for name in ("stiffness", "damping"):
        ...
        factor = torch.tensor(
            np.exp(
                rng.uniform(
                    np.log(cfg["gain_scale"][0]), np.log(cfg["gain_scale"][1]), nominal.shape
                )
            ),
            dtype=nominal.dtype,
        )
        if cfg.get("hand_gain_joint_pattern"):
            ...
            selected = torch.tensor(
                [bool(re.fullmatch(cfg["hand_gain_joint_pattern"], n)) for n in env.robot.dof_names]
            )
            factor[:, ~selected] = 1.0
    ...
    env.default_q_offset[:] = torch.tensor(
        rng.uniform(-amount, amount, (env.num_envs, 6)), device=env.device, dtype=torch.float32
    )
    ...
    env.randomized_physics["hand"]["materials"] = (
        env.robot._physics_view.get_material_properties().clone().cpu()
    )
...
def property_report(env):
    ...
    for name, view in [
        ...
        report[name] = dict(
            static_friction_range=[float(m[..., 0].min()), float(m[..., 0].max())],
            dynamic_friction_range=[float(m[..., 1].min()), float(m[..., 1].max())],
            mass_range_kg=[float(mass.min()), float(mass.max())],
        )
    com = (
        env.can._physics_view.get_coms().cpu()[..., :3]
        - env.nominal_physics["object"]["coms"][..., :3]
    )
    report["object"]["com_offset_ranges_m"] = torch.stack((com.amin(0), com.amax(0))).tolist()
    report["joint_default_offset_ranges_rad"] = (
        torch.stack((env.default_q_offset.amin(0), env.default_q_offset.amax(0))).cpu().tolist()
    )
...
class PushSchedule:
    ...
    def reset(self, ids):
        self.remaining[ids] = uniform(
            (len(ids), 2), *self.cfg["interval_s"], self.device, self.generator
        )

    def advance(self, env):
        self.last_delta.zero_()
        if not env.training or not self.cfg["enabled"]:
            return
        ...
        for slot, asset in enumerate((env.robot, env.can)):
            ...
            self.remaining[ids, slot] = uniform(
                (len(ids),), *self.cfg["interval_s"], self.device, self.generator
            )
            if stage[1] > 0:
                self.total_events += len(ids)
