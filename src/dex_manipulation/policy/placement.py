...
class RandomCanPlacement:
    ...
    def __init__(self, candidates, metadata, seed=None):
        ...
        self.metadata = dict(metadata, enabled=True, seed=self.seed, count=len(candidates),
...
def load_random_placement(root, config, arm_config, speed, seed=None, reference=None):
    ...
    """
    root = Path(root)
    catalog = json.loads((root / 'config/play.json').read_text())
    demo = catalog['demos']['2']
    known = [json.loads((root / demo[key]).read_text())['reference']
             for key in ('config', 'training_config') if key in demo]
    if (config.get('arm_training', {}).get('enabled') or
            resolve_demo_path(config['reference'], root).resolve() not in
    ...
    if path is None:
        raise ValueError('--random-can requires a verified IK map for this speed (available: ' + ', '.join(maps) + ')')
    directory = root / path
    if not (directory / 'manifest.json').is_file():
        raise ValueError(f'Verified IK region is missing: {directory}. Restore or regenerate the tabletop IK report.')
    ...
    scanned_config = json.loads((root / manifest['config_path']).read_text())
    if arm_config != scanned_config or arm_config['solver'] != manifest['solver']:
        raise ValueError('IK region arm placement/solver configuration differs')
    for path in (manifest['config_path'], arm_config['input'], arm_config['workcell'],
                 arm_config['alignment'], arm_config['arm_model'], arm_config['usd'],
                 config['model'], config['object_geometry']):
        if path not in hashes:
            raise ValueError(f'IK region does not cover this input: {path}')
    geometry = json.loads((directory / 'collision_model.json').read_text())
    ...
    source = ReferenceMotion(root / arm_config['input'], hand, root / config['object_geometry'], config['world_frame'])
    if reference is None:
        ...
        if 'reference_timing' in config:
            ...
            reference.retime_for_control_horizon(config['episode_length_s'], config['physics_dt'] * config['control_decimation'])
        reference = reference.for_playback(speed, config['physics_dt'] * config['control_decimation'])
    if (reference.metadata['frame_ids'] != manifest['source_frames'] or
            reference.object.shape != source.object.shape or
            not np.allclose(reference.object, source.object, atol=1e-9, rtol=0) or
            not np.allclose(reference.wrist[0], source.wrist[0], atol=1e-9, rtol=0) or
            not np.allclose(reference.q[0], source.q[0], atol=1e-9, rtol=0) or
            not np.isclose(reference.duration, manifest['reference_duration_s'], atol=1e-8, rtol=0)):
        raise ValueError('Policy initial hand pose, object trajectory or timing differs from the verified IK region')
    ...
    nominal = np.asarray(workcell.resolve_alignment(json.loads((root / arm_config['alignment']).read_text()))['world_from_source'])
    grid = json.loads((directory / 'grid.json').read_text())
    candidates = []
    for row in grid:
        if not (row['initial_can_supported'] and row['ik_success'] and row.get('collision_free')
        ...
        with np.load(directory / 'points' / f"{row['ix']:02}_{row['iy']:02}.npz", allow_pickle=False) as saved:
            checked_pose(saved['world_from_source'])
            if (not saved['success'].all() or not saved['tested'].all() or
                    not saved['transition_valid'][1:].all() or
                    not np.allclose(world, saved['world_from_source'], atol=1e-9, rtol=0)):
    ...
    return RandomCanPlacement(candidates, dict(
        demo='2', region=str(directory), region_manifest_sha256=digest(directory / 'manifest.json'),
        region_grid_sha256=digest(directory / 'grid.json'), grid_step_m=manifest['grid_step_m'],
        playback_speed=speed, policy_reference_sha256=reference.metadata['reference_sha256'],
        reference_start_checked=True,
        interpretation='Retarget IK/collision/singularity-checked grid and identical policy start; residual policy motion and physical grasp are not certified'), seed)
