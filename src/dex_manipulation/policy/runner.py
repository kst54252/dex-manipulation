...
def _evaluate_episode_batch(env, learner, output, label, seed, zero=False):
    ...
    first_done = None
    for step in range(int(np.ceil(env.reference.duration / env.task.dt)) + 1):
        if zero:
        ...
        for i in np.flatnonzero(done):
            success[i] = bool(not metrics["early_failure"][i] and metrics["demo_end"][i] and constraints_ok[i] and max_error[i] < env.cfg["success_object_error_m"]
    ...
    arrays["valid"] = arrays["joint_constraints_valid"] & ~arrays["metric_early_failure"]
    ...
    report = dict(label=label, seed=seed, episodes=n, success_count=int(success.sum()),
                  ...
                  success_definition="full sequence, no early failure, every-step object 50-keypoint mean error <25mm, final height error <25mm, and recorded joint/coupling tolerances",
                  ...
                  valid_mask_definition="finite simulated step, no early termination, joint/coupling numerical tolerances; not a nonpenetration or grasp certificate")
...
def plot_comparison(output):
    ...
    for ax in axes:
        ax.grid(alpha=0.25)
    fig.suptitle("Dynamic PhysX evaluation: curves end on failure; reference uses configured episode timing")
    fig.tight_layout()
    fig.savefig(Path(output) / "evaluation.png", dpi=150)
    plt.close(fig)


def run(args,root,on_ready=None):
    print('[startup] load configuration and reference',flush=True)
    config=json.loads(Path(args.config).read_text())
    config['source_profile']=args.source_profile
    if args.source_profile=='wuji':
    ...
    metadata=dict(config=config,reference=reference.metadata,model_sha256=digest(root/config['model']),
                  observation_schema='regrind_revo2_v3_actor67_critic88',implementation='independent task and simulator adapter; general RSL-RL PPO; no REGRIND imports')
    from ..scene import FloatingSurface
    metadata['surface']=FloatingSurface(config['surface'],reference.object[0,:3,3]).metadata()
    metadata['contract_hash']=hashlib.sha256(json.dumps(metadata,sort_keys=True).encode()).hexdigest()
    ...
    try:
        ...
        comparison=dict(training_steps=learner.steps,zero_residual_success=baseline['success_count'],trained_success=trained['success_count'],episodes=env.num_envs,
            ...
            limitations=['single sequence','assumed table/world frame','idealized floating wrist','original capture fps unknown'],training_rsi=config['rsi'],training_gravity_curriculum=config['gravity_curriculum'],training_augmentation=config['augmentation'])
