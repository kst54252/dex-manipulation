...
class TensorReference:
    def __init__(self, reference, num_envs, config, device, *, velocity_mode='segment', control_dt=None):
        ...
        self.start,self.end=config.get('blend_start_s',0.),config.get('blend_end_s',self.duration)
        ...
        if velocity_mode=='control_difference':
            ...
            prev=np.maximum(np.arange(count)-1,0);nxt=np.minimum(np.arange(count)+1,count-1)
            span=((nxt-prev)*control_dt)[:,None]
            values={}
            for name in ('q','wrist','object'):
                x=grid['q' if name=='q' else name+'_position']
                v=(x[nxt]-x[prev])/span;v[0]=0
                values[name+'_velocity']=v
                if name!='q':
                    rotation=Rotation.from_quat(grid[name+'_quaternion'])
                    omega=(rotation[nxt]*rotation[prev].inv()).as_rotvec()/span;omega[0]=0
                    values[name+'_angular_velocity']=omega
            self.velocity_grid={k:torch.as_tensor(v,dtype=torch.float32,device=device) for k,v in values.items()}
    ...
    def sample(self, time, env_ids=None):
        ...
        if self.velocity_grid is not None:
            ...
            left=coordinate.floor().long();right=(left+1).clamp(max=len(self.velocity_grid['q_velocity'])-1)
            blend=(coordinate-left)[:,None]
            for name,values in self.velocity_grid.items():out[name]=torch.lerp(values[left],values[right],blend)
        w = torch.ones_like(t) if self.mode=='rigid_sequence' else ((self.end-t)/(self.end-self.start)).clamp(0, 1)
        dw = torch.zeros_like(t) if self.mode=='rigid_sequence' else torch.where((t>=self.start)&(t<self.end), -1/(self.end-self.start), 0.)
