...
No base task, assets, control law, observations or existing configs are changed.
This adapter is installed only by the dedicated demo2 entry point.
"""
import math
import numpy as np
import torch
...
def contact_schedule(reference, settings):
    ...
    return dict(start_frame_id=int(ids[found[0]]), start_time_s=start,
                end_frame_id=int(ids[-1]), end_time_s=end,
...
def contact_terms(forces, time, object_error, early_failure, settings, schedule):
    ...
    on = ((time + 1e-6 >= schedule['start_time_s']) &
          (time <= schedule['end_time_s'] + 1e-6)).float()
    ...
    terms = dict(grasp_fingers=on * quality * settings['finger_weight'] * onset,
    ...
    metrics = dict(grasp_active=on, grasp_contact_count=duty.sum(-1),
...
class PadCanContacts:
    ...
    def __init__(self, env):
        ...
        if self.view.sensor_count != 5 * env.num_envs or self.view.filter_count != 1:
            raise RuntimeError('Expected exactly five pad sensors and one can filter per environment')
        ...
        if actual_filters != filters:
            ...
            if actual_filters != [f[0] for f in filters]:
                raise RuntimeError('PhysX can filter mapping differs from the requested explicit pairs')
        self.metadata = dict(schema=SCHEMA, finger_order=list(FINGERS), pad_bodies=list(PAD_BODIES),
            sensors=self.view.sensor_count, filters_per_sensor=self.view.filter_count,
            force_unit='N', impulse_dt_s=env.cfg['physics_dt'],
    ...
    def consume(self):
        ...
        if len(self.samples) != expected:
            raise RuntimeError(f'Expected {expected} contact substeps; received {len(self.samples)}')
...
class ContactTask:
    ...
    def score(self, state, ref, *args, **kwargs):
        ...
        extra, contact = contact_terms(self.contacts.consume(), ref['time'],
            metrics['object_keypoint_error_m'], metrics['early_failure'], self.settings, self.schedule)
...
def attach_contact_reward(env):
    ...
    env.metadata['five_finger_grasp'] = dict(schedule=schedule, sensor=contacts.metadata, settings=settings)
    return contacts
