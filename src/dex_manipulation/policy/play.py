...
def play(env, learner, output, episodes=0, is_running=None, protocol="strict"):
    ...
    manifest = dict(
        checkpoint_iteration=learner.iteration,
        training=False,
        num_envs=1,
        render=env.render,
        deterministic=True,
        ...
        hand_gravity=env.cfg["hand_gravity"],
        self_collision=env.cfg["self_collision"],
        rsi=False,
    ...
    with (output / "episodes.jsonl").open("w") as log, torch.no_grad():
        while running() and not stopped and (episodes == 0 or episode < episodes):
            ...
            if placement is not None:
                print(
                    "[play placement]",
                    json.dumps(dict(episode=episode + 1, **placement)),
                    flush=True,
                )
                if episode == 0:
            ...
            while running():
                ...
                table_ok = metrics.get("table_clearance_m", float("inf")) >= 0.0
                constraints_ok &= table_ok
                if episode == 0:
                    rows.append(
                        dict(
                            time_s=float(info["reference_time"][0]),
                            ...
                            tracking_within_tolerance=metrics["object_keypoint_error_m"]
                            < env.cfg["success_object_error_m"],
                            action=numpy_tree(action[0]),
                            **{k: numpy_tree(v[0]) for k, v in info["state"].items()},
                            **{
                                "reference_" + k: numpy_tree(v[0])
                                for k, v in info["reference"].items()
                            },
                            **{
                                "raw_target_" + k: numpy_tree(v[0])
                                for k, v in info["raw_targets"].items()
                            },
                            **{
                                "applied_target_" + k: numpy_tree(v[0])
                                for k, v in info["applied_targets"].items()
                            },
            ...
            success = (
                ...
                and not metrics["early_failure"]
                and bool(metrics["demo_end"])
                and constraints_ok
                and max_error < env.cfg["success_object_error_m"]
            ...
            reason = (
                ...
                if metrics.get("arm_ik_success", 1.0) < 0.5
                else "early_failure"
                if metrics["early_failure"]
                else "demo_end"
                if metrics["demo_end"]
                else "timeout"
            )
            report = dict(
                ...
                tracking_success=bool(success),
                constraints_passed=bool(constraints_ok),
                maximum_object_error_m=max_error,
