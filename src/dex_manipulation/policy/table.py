...
def compose(a, b):
    ...
    return torch.cat((a[..., :3] + quat_apply(aq, b[..., :3].expand_as(aq[..., :3])),
                      quat_multiply(aq, bq)), -1)
...
class TableClearance:
    def __init__(self, model, body_names, config, device='cpu'):
        ...
        self.margin = float(config.get('clearance_m', .004))
        self.lookahead = float(config.get('lookahead_s', .02))
        if not math.isfinite(self.margin) or self.margin <= 0 or not 0 <= self.lookahead <= .2:
            raise ValueError('Invalid table clearance/lookahead')
        if any(not math.isfinite(float(config.get(k,0))) or float(config.get(k,0))<0
        ...
        tensor = lambda v: torch.as_tensor(np.asarray(v), device=device, dtype=torch.float32)
        ...
        for c in model.colliders:
            ...
            lows.append(vertices.min(0)); highs.append(vertices.max(0))
        ...
        for j in model.joints:
            ...
            self.frames.append((frame('frame0'), invert(frame('frame1')),
                                tensor(j.get('axis', [0., 0., 0.])),
    ...
    def fk(self, target):
        ...
        for j, (f0, f1, axis, index) in zip(self.model.joints, self.frames):
            motion = torch.zeros_like(root); motion[:, 6] = 1.
            if j['kind'] == 'revolute':
    ...
    def guard(self, target, state):
        ...
        lift = torch.maximum(self.margin-command_lower,
                             self.margin-predicted-current_to_target).clamp_min(0)
        position = target['position'].clone(); position[:, 2] += lift
        self.maximum_lift = torch.maximum(self.maximum_lift, lift)
        return dict(target, position=position)

    def metrics(self):
        return dict(table_clearance_m=self.minimum,
