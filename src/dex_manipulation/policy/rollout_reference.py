...
def _transforms(position, quaternion):
    ...
    if (
        ...
        or np.any(abs(np.linalg.norm(q, axis=-1) - 1) > 1e-3)
    ):
        raise ValueError("Invalid measured pose")
    out = np.tile(np.eye(4), (len(p), 1, 1))
    out[:, :3, 3] = p
    out[:, :3, :3] = Rotation.from_quat(q).as_matrix()
    return out
...
def align_contact_poses(data, rollout, environment=0):
    ...
    def all_values(key):
        return rollout[key][:, environment] if batched else rollout[key]
    ...
    indices = np.arange(n)
    if len(rollout["q"]) != n:
        if "time_s" not in rollout:
            raise ValueError("Dense rollouts require command timestamps")
        time = all_values("time_s")
        if len(time) < n or np.any(np.diff(time) <= 0):
            raise ValueError("Invalid rollout timestamps")
    ...
    return data["object_transform"] @ np.linalg.inv(actual_object) @ actual_wrist, values(
        "q"
    ).copy()


def prepare_contact_reference(
    source,
    rollout_path,
    output,
    model,
    ...
    *,
    environment=0,
    clearance_m=0.0001,
    projection_translation_m=0.002,
    projection_rotation_rad=0.02,
    projection_joint_rad=0.02,
    ...
    if output.suffix != ".npz":
        raise ValueError("The output filename must end in .npz")
    ...
    parameters = [
        ...
        projection_rotation_rad,
        projection_joint_rad,
    ]
    ...
    with np.load(rollout_path, allow_pickle=False) as f:
        rollout = {k: f[k].copy() for k in f.files}
    ...
    scale = np.array(
        [projection_translation_m] * 3 + [projection_rotation_rad] * 3 + [projection_joint_rad] * 6
    )
    records = []
    ...
    def clearance(j, t, obj):
        ...
        ground = (
            min(
                (np.asarray(c["vertices"]) @ links[c["link"]][:3, :3].T + links[c["link"]][:3, 3])[
                    :, 2
                ].min()
    ...
    for i in range(len(q)):
        ...
        def constraints(x):
            t, a = pose(x)
            ds, z = clearance(a, t, obj)
            return np.r_[ds - clearance_m, z] / projection_translation_m

        bounds = [(-1.0, 1.0)] * 6 + list(
            zip(
                np.maximum(-1, (model.lower - j) / scale[6:]),
                np.minimum(1, (model.upper - j) / scale[6:]),
            )
        )
        result = minimize(
            lambda x: float(x @ x),
        ...
        feasible = bool(
            ...
            and ds.min() >= clearance_m * 0.9
            and z >= 0
            and model.limit_violation(a) <= 1e-8
        )
        records.append(
            dict(
                ...
                geometry_valid=feasible,
                minimum_distance_m=float(ds.min()),
                ground_clearance_m=z,
                projection=result.x.tolist(),
            )
        )
        if not feasible:
            raise ValueError(f"Contact projection failed: {records[-1]}")
        data["wrist_transform"][i] = t
        data["active_q_rad"][i] = a
        data["full_q_rad"][i] = model.expand(a)
        data["wrist_translation_m"][i] = t[:3, 3]
        data["wrist_quaternion_xyzw"][i] = Rotation.from_matrix(t[:3, :3]).as_quat()
        data["robot_keypoints"][i] = model.keypoint_positions(a, t)
        data["valid"][i] = True
    transition = []
    for i in range(1, len(q)):
        keys = ["wrist_transform", "object_transform"]
        rotations = [
            Slerp([0, 1], Rotation.from_matrix(data[k][i - 1 : i + 1, :3, :3])) for k in keys
        ]
        ...
        for fraction in np.linspace(0, 1, transition_substeps + 1):
            ...
            for key, rot in zip(keys, rotations):
                t = np.eye(4)
                t[:3, :3] = rot(fraction).as_matrix()
                t[:3, 3] = (1 - fraction) * data[key][i - 1, :3, 3] + fraction * data[key][i, :3, 3]
                poses.append(t)
            j = (1 - fraction) * data["active_q_rad"][i - 1] + fraction * data["active_q_rad"][i]
            ds, z = clearance(j, poses[0], poses[1])
            minimum = min(minimum, float(ds.min()))
            ground_minimum = min(ground_minimum, z)
        ...
        transition.append(
            dict(
                ...
                ground_clearance_m=ground_minimum,
                valid=valid,
            )
        )
        if not valid:
            raise ValueError(f"Contact transition failed: {transition[-1]}")
    velocity = np.abs(np.diff(data["full_q_rad"], axis=0) / np.diff(data["timestamps_s"])[:, None])
    if np.any(velocity > model.full_velocity + 1e-6):
        raise ValueError("Prepared reference exceeds input-time joint velocity limits")
    metadata = dict(
        method="own physical hand/object contact alignment with fixed original object targets",
        source=str(source),
        source_sha256=_digest(source),
        rollout=str(rollout_path),
        rollout_sha256=_digest(rollout_path),
        environment_index=environment,
        object_targets_changed=False,
        timestamps_changed=False,
        clearance_m=clearance_m,
        projection_scales=scale.tolist(),
        transition_substeps=transition_substeps,
        frames=records,
        transitions=transition,
        limitations=[
            "sampled hand-object and hand-ground geometry checks, not continuous collision proof",
