...
def frozen_source(rollout, reference, control_dt, kind='command', time_scale=1.):
    ...
    if (len(rollout['physics_time_s']) != count or
            not np.allclose(rollout['physics_time_s'], expected, atol=1e-6, rtol=0) or
            not np.allclose(rollout['time_s'], np.arange(count)*control_dt, atol=2e-6, rtol=0)):
        raise ValueError('Capture must contain every command from frame zero through demo end')
    p, r, q = (('raw_target_position', 'raw_target_quaternion', 'raw_target_active_q')
               if kind == 'command' else ('wrist_position', 'wrist_quaternion', 'q'))
    positions, quaternions, joints = (np.asarray(rollout[k]) for k in (p, r, q))
    if (positions.shape != (count, 3) or quaternions.shape != (count, 4) or
            joints.shape != (count, 6) or not all(np.isfinite(v).all() for v in (positions, quaternions, joints)) or
            not np.allclose(np.linalg.norm(quaternions, axis=1), 1., atol=1e-5)):
    ...
    return dict(timestamps_s=np.r_[0., expected]*time_scale, wrist_transform=wrist,
                object_transform=obj, active_q_rad=np.vstack([reference.q[0], joints]),
                active_joint_names=np.array(reference.model.active_names),
                frame_ids=np.arange(count+1), valid=np.ones(count+1, bool),
                transition_valid=np.ones(count+1, bool),
                object_geometry_fingerprint=np.array(reference.metadata['object_geometry_fingerprint']))


def plan(root, capture, arm_config_path, output, kind='command', time_scale=1.):
    ...
    metadata = json.loads((capture/'run_metadata.json').read_text())
    playback = json.loads((capture/'playback.json').read_text())
    ...
    reference = ReferenceMotion(root/config['reference'], hand, root/config['object_geometry'], config['world_frame'])
    ...
    with np.load(capture/'first_episode.npz', allow_pickle=False) as data:
        source = frozen_source(data, reference, config['physics_dt']*config['control_decimation'], kind, time_scale)
    arm_config = json.loads(Path(arm_config_path).read_text())
    arm = ArmModel.load(root/arm_config['arm_model'])
    workcell = Workcell.load(root/arm_config['workcell'])
    alignment = workcell.resolve_alignment(json.loads((root/arm_config['alignment']).read_text()))
    ...
    report = solve_trajectory(arm, hand, source_path, output, alignment['base_from_source'],
                              arm_config['seed_q_rad'], IKOptions(**arm_config['solver']),
                              alignment_status='frozen_policy_simulation_diagnostic',
                              trajectory_substeps=substeps)
    manifest = dict(schema='frozen_policy_tracking_v1', capture=str(capture.resolve()),
                    capture_sha256=digest(capture/'first_episode.npz'),
                    policy_contract_hash=metadata['contract_hash'], policy_iteration=playback['checkpoint_iteration'],
                    config=config, arm_config=arm_config, arm_config_sha256=digest(arm_config_path),
                    kind=kind, time_scale=time_scale, source_reference_duration_s=reference.duration,
                    ...
                    command_semantics=('Frozen actor PD pose and finger position targets' if kind == 'command'
...
def stats(values):
    ...
    return dict(mean=float(values.mean()), maximum=float(values.max()), p95=float(np.percentile(values, 95)))


def replay(env, metadata, root, plan_path, checkpoint_path, output, repeats=3, hold_s=1., velocity_feedforward=True):
    ...
    manifest = json.loads((plan_path.parent/'frozen.json').read_text())
    ...
    if not (saved['metadata']['contract_hash'] == manifest['policy_contract_hash'] == metadata['contract_hash']):
        raise ValueError('Capture/checkpoint/physics configuration mismatch')
    if (model_fingerprint(env.arm) != reference.metadata['arm_fingerprint'] or
            model_fingerprint(env.model) != reference.metadata['hand_fingerprint'] or
            not np.allclose(env.world_from_source, manifest['world_from_source'], atol=1e-10, rtol=0)):
    ...
    for repeat in range(repeats):
        ...
        for step in range(count+hold_count):
            ...
            env.robot.set_joint_position_targets(torch.tensor(qa[i:i+1], device=env.device, dtype=torch.float32), joint_indices=env.arm_ids)
            env.robot.set_joint_velocity_targets(torch.tensor(arm_velocity[None], device=env.device, dtype=torch.float32), joint_indices=env.arm_ids)
            env.robot.set_joint_position_targets(torch.tensor(full[i:i+1], device=env.device, dtype=torch.float32), joint_indices=env.full_ids)
            ...
            for substep in range(env.cfg['control_decimation']):
                ...
                actual = base_from_source @ transform(Rotation.from_quat(s['wrist_quaternion']).as_matrix(), s['wrist_position'])
                ...
                obj = base_from_source @ transform(Rotation.from_quat(s['object_quaternion']).as_matrix(), s['object_position'])
                points = env.reference.object_local
                expected_points = points @ d['object_transform'][i,:3,:3].T + d['object_transform'][i,:3,3]
                actual_points = points @ obj[:3,:3].T + obj[:3,3]
                rows.append(dict(timestamp_s=(step*env.cfg['control_decimation']+substep+1)*env.cfg['physics_dt'],
                    target_timestamp_s=reference.times[i], command_index=i,
                    control_endpoint=substep==env.cfg['control_decimation']-1, hold=hold,
                    q_arm=s['q_arm'], q_arm_velocity=s['q_arm_velocity'], q_finger=s['q'],
                    q_full=s['full_q'], target_q_arm=qa[i], target_q_finger=qf[i],
                    target_q_arm_velocity=arm_velocity, wrist_target=d['wrist_target_pose'][i],
                    wrist_actual=actual, fk_actual=fk, object_actual=obj,
                    ...
                    object_error_mm=np.linalg.norm(actual_points-expected_points,axis=1).mean()*1000,
                    sigma_min=sigma, condition=condition,
                    arm_limit_violation_rad=max(0.,np.max(env.arm.lower-s['q_arm']),np.max(s['q_arm']-env.arm.upper)),
                    arm_velocity_violation_rad_s=np.maximum(np.abs(s['q_arm_velocity'])-env.arm.velocity,0).max(),
                    finger_velocity_violation_rad_s=np.maximum(np.abs(s['full_q_velocity'])-env.model.full_velocity,0).max(),
                    coupling_error_rad=np.abs(s['full_q']-s['q']@env.model.coupling.T-env.model.offset).max()))
        ...
        summary = dict(repeat=repeat+1, command_count=count, physics_samples=int(motion.sum()),
                       policy_inferences=0, online_ik_calls=0, object_writes_during_motion=0,
                       endpoint={k:stats(arrays[k][end]) for k in ('position_error_mm','orientation_error_deg','object_error_mm')},
                       all_physics_ticks={k:stats(arrays[k][motion]) for k in ('position_error_mm','orientation_error_deg')},
                       per_arm_joint_error_deg={n:stats(np.rad2deg(np.abs(arrays['q_arm'][end,j]-arrays['target_q_arm'][end,j]))) for j,n in enumerate(env.arm.active_names)},
                       per_finger_joint_error_deg={n:stats(np.rad2deg(np.abs(arrays['q_finger'][end,j]-arrays['target_q_finger'][end,j]))) for j,n in enumerate(env.model.active_names)},
                       maximum_fk_consistency_mm=float(arrays['fk_consistency_mm'].max()),
                       min_sigma=float(arrays['sigma_min'].min()), max_condition=float(arrays['condition'].max()),
                       near_singular_sample_indices=np.flatnonzero((arrays['sigma_min']<env.bridge.solver.options.near_sigma_min)|(arrays['condition']>env.bridge.solver.options.near_condition)).tolist(),
                       maximum_arm_limit_violation_rad=float(arrays['arm_limit_violation_rad'].max()),
                       maximum_arm_velocity_violation_rad_s=float(arrays['arm_velocity_violation_rad_s'].max()),
                       maximum_finger_velocity_violation_rad_s=float(arrays['finger_velocity_violation_rad_s'].max()),
                       maximum_coupling_error_rad=float(arrays['coupling_error_rad'].max()))
        if hold_count:
            summary['hold_final']={k:float(arrays[k][-1]) for k in ('position_error_mm','orientation_error_deg','object_error_mm')}
    ...
    report=dict(schema='frozen_tracking_report_v1', plan=str(plan_path.resolve()),
                plan_sha256=digest(plan_path), checkpoint_sha256=digest(checkpoint_path),
                policy_contract_hash=manifest['policy_contract_hash'], kind=manifest['kind'],time_scale=manifest['time_scale'],
                duration_s=float(reference.times[-1]),hold_s=hold_count*dt,
                velocity_feedforward=velocity_feedforward, physics=env.metadata,
                policy_inferences_during_replay=0, online_ik_during_replay=0,
