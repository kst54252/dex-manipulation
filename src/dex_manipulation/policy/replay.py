...
def write_replay(output, model, reference):
    ...
    for i, q in enumerate(sampled["q"]):
        wrist = transform(
            Rotation.from_quat(sampled["wrist_quaternion"][i]).as_matrix(),
            sampled["wrist_position"][i],
        )
        links = model.link_transforms(q, wrist)
        poses.append(
            [
                np.r_[links[n][:3, 3], Rotation.from_matrix(links[n][:3, :3]).as_quat()].tolist()
                for n in link_names
            ]
        )
    human = (
        reference.data["human_keypoints"] @ reference.camera_to_world[:3, :3].T
        + reference.camera_to_world[:3, 3]
    )
    human_sampled = np.stack(
        [
            np.interp(times, reference.times, human[:, k, xyz])
            for k in range(21)
            for xyz in range(3)
        ],
        axis=-1,
    ...
    tracks = [
        dict(
            label="기준 궤적 · 사람 21점 + 기하학적 Revo2",
            times=times,
            body_names=link_names,
            links=poses,
            object=np.c_[sampled["object_position"], sampled["object_quaternion"]],
            human=human_sampled,
            status="기하학적 reference; 물리 성공 아님",
        )
    ]
    for stem, label in (
        ("zero_residual", "무보정 · 실제 PhysX"),
        ("trained_residual", "Residual PPO · 실제 PhysX"),
    ):
        with np.load(output / f"{stem}_rollout.npz", allow_pickle=False) as d:
            report = read_config(output / f"{stem}_evaluation.json")
            tracks.append(
                dict(
                    label=label,
                    times=d["time_s"],
                    body_names=d["body_names"],
                    links=d["link_transforms"],
    ...
    payload = dict(
        ...
        collision_shapes=reference.collision_shapes,
        geometry=[
            dict(link=c["link"], vertices=c["vertices"], faces=c["faces"]) for c in model.colliders
        ],
    ...
    html = (
        """<!doctype html><html lang="ko"><meta charset="utf-8"><title>Residual RL — measured PhysX replay</title>
