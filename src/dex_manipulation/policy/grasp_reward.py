...
from .contact_reward import SCHEMA, ContactWorld, PadCanContacts, contact_schedule, contact_terms
...
class PadApproach:
    def __init__(self, env):
        self.ids = torch.tensor(
            [env.robot.body_names.index(n) for n in PAD_BODIES], device=env.device
        )
        samples = []
        for name in PAD_BODIES:
            collider = next(c for c in env.model.colliders if c["link"] == name)
            vertices = np.array(collider["vertices"])
            faces = np.array(collider["faces"])
        ...
        for shape in env.reference.object_geometry["collision_shapes"]:
            ...
            self.shapes.append(
                (
                    torch.tensor(transform[:3, 3], device=env.device, dtype=torch.float32),
                    shape["radius"],
                    shape["height"] / 2,
                )
            )
    ...
    def gap(self, state):
        ...
        points = (
            quat_apply(
                pose[:, :, None, 3:].expand(*shape[:-1], 4), self.samples[None].expand(shape)
            )
            + pose[:, :, None, :3]
        )
        local = quat_apply(
            quat_inverse(state["object_quaternion"])[:, None, None].expand(*shape[:-1], 4),
...
def approach_terms(gap, forces, time, object_error, early_failure, settings, schedule):
    start = schedule["start_time_s"]
    lead = settings["approach_lead_s"]
    ...
    return proximity + weakest + opposed + pressure_bonus, dict(
        reward_pad_approach=proximity,
        reward_pad_weakest=weakest,
        reward_pad_opposition=opposed,
        reward_pad_pressure=pressure_bonus,
        pad_mean_gap_m=gap.mean(-1),
        pad_max_gap_m=gap.amax(-1),
        pad_thumb_gap_m=gap[:, 0],
        grasp_opposition_duty=opposition,
    )
...
class GraspTask:
    ...
    def targets(self, ref, actions, previous_q=None, default_offset=None):
        ...
        lead = self.settings["preload_ramp_s"]
        start = self.schedule["start_time_s"]
        alpha = ((ref["time"] - (start - lead)) / lead).clamp(0, 1)
        alpha = alpha * alpha * (3 - 2 * alpha)
        target["active_q"] = (target["active_q"] + alpha[:, None] * self.preload).clamp(
            self.base.lower, self.base.upper
        )
    ...
    def score(self, state, ref, *args, **kwargs):
        ...
        dense, proximity = approach_terms(
            self.approach.gap(state),
            forces,
            ref["time"],
            metrics["object_keypoint_error_m"],
            metrics["early_failure"],
...
def attach_grasp_task(env):
    ...
    if preload.shape != (6,) or not np.isfinite(preload).all() or np.abs(preload).max() > 0.1:
    ...
    env.metadata["grasp_task"] = dict(
        settings=settings,
        schedule=schedule,
        contact_sensor=contacts.metadata,
