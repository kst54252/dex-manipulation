...
def build_arm_scene(
    ...
    *,
    ...
    robot_path, workcell_path, can_path = (
        prefix + "/Robot",
        prefix + "/Workcell",
        prefix + "/DynamicCan",
    )
    ...
    UsdPhysics.MassAPI.Apply(table_prim).CreateMassAttr(1.0)
    ...
    table_physics.CreateMaxDepenetrationVelocityAttr(1.0)
    ...
    for joint in model.moving:
        ...
        scale = np.pi / 180 if config.get("joint_gain_units") == "si" else 1.0
        driven = not joint.get("mimic") or config.get("mimic_mode") == "constraint_with_drives"
        drive.CreateStiffnessAttr(config["joint_stiffness"] * scale if driven else 0.0)
        drive.CreateDampingAttr(config["joint_damping"] * scale if driven else 0.0)
        ...
        if joint.get("mimic"):
            ...
            if policy == "asset":
                if "NewtonMimicAPI" not in prim.GetAppliedSchemas():
                    raise ValueError(
                        "The native hand mimic schema is not registered in this Isaac runtime"
                    )
                ...
                mimic.CreateReferenceJointAxisAttr(
                    "rot" + str(UsdPhysics.RevoluteJoint(leader).GetAxisAttr().Get())
                )
                mimic.CreateGearingAttr(-spec["multiplier"])
                mimic.CreateOffsetAttr(-np.rad2deg(spec["offset"]))
            else:
                raise ValueError(
                    "Arm policy play requires one explicit or native mimic representation"
                )
    hand_links = {model.root} | {j["child"] for j in model.description["joints"]}
    for prim in prims():
        if prim.HasAPI(UsdPhysics.RigidBodyAPI):
            rb = PhysxSchema.PhysxRigidBodyAPI.Apply(prim)
            rb.CreateDisableGravityAttr(
                not config["hand_gravity"] if prim.GetName() in hand_links else False
            )
            if prim.GetName() in hand_links:
                rb.CreateMaxDepenetrationVelocityAttr(
                    config["motion_control"]["contact_correction_velocity_m_s"]
                )
    ...
    table_collision.CreateContactOffsetAttr(0.005)
    table_collision.CreateRestOffsetAttr(0.0)
    table_collision.CreateTorsionalPatchRadiusAttr(
        config.get("table_torsional_patch_radius_m", 0.0)
    )
    table_collision.CreateMinTorsionalPatchRadiusAttr(
        config.get("table_min_torsional_patch_radius_m", 0.0)
    )
    wrist_path = robot_path + arm_config["wrist_path"][len(source_root) :]
    articulation_root, articulation_resolution = prepare_arm_articulation(
        stage, robot_path, base_path, wrist_path
    )
    ...
    UsdShade.MaterialBindingAPI.Apply(stage.GetPrimAtPath(can_path)).Bind(
        material, materialPurpose="physics"
    )
    ...
    can_xform.AddScaleOp().Set(Gf.Vec3f(1.0))
    return dict(
        robot=str(articulation_root.GetPath()),
        can=can_path,
        table=str(table_prim.GetPath()),
        resolution=articulation_resolution,
        initial_can_pose=initial_can_pose,
        pad_material_binding=pad_material,
    )
