...
class ObservationHistory:
    def __init__(self, count, dofs, config, device, generator):
        ...
        self.actor_size, self.critic_size = 43+4*dofs, 43+5*dofs+15
    ...
    def update(self, state, ids, reset=False, noisy=True):
        ...
        values = dict(policy_position=self.delayed["wrist_position"][ids],policy_rotation=rotation6d(self.delayed["wrist_quaternion"][ids]),policy_q=self.delayed["q"][ids],
                      critic_position=state["wrist_position"][ids],critic_rotation=rotation6d(state["wrist_quaternion"][ids]),critic_q=state["q"][ids])
    ...
    def assemble(self,state,ref,action,phase,ids=None):
        ...
        common=[action[ids],phase[ids,None],ref["wrist_position"][ids],rotation6d(ref["wrist_quaternion"][ids]),ref["q"][ids]]
        def history(prefix):
        ...
        self.critic[ids]=torch.cat([state["object_position"][ids],rotation6d(state["object_quaternion"][ids]),*history("critic"),*common,state["fingertips"][ids].flatten(1),state["q_velocity"][ids]],-1)
        return self.get()
