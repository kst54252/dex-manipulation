"""Hand/object observation history and additional measured arm context."""

import torch
from tensordict import TensorDict
from dex_manipulation.policy.math3d import rotation6d, from_rotvec, quat_multiply, slerp, uniform


class ObservationHistory:
    def __init__(self, count, dofs, config, device, generator):
        self.count, self.dofs, self.cfg, self.device, self.generator = (
            count,
            dofs,
            config,
            device,
        ...
        self.actor_size, self.critic_size = (
            43 + 4 * dofs,
            43 + 5 * dofs + 15 + 6 * int(self.include_object_velocity),
        )
        ...
        self.buffers = {
            name: torch.zeros((count, 3, width), device=device)
            for name, width in (
                ("object_position", 3),
                ("object_quaternion", 4),
                ("wrist_position", 3),
        ...
        self.histories = {
            name: torch.zeros((count, 2, width), device=device)
            for name, width in (
                ("policy_position", 3),
                ("policy_rotation", 6),
                ("policy_q", dofs),
    ...
    def reset_lags(self, ids, noisy):
        self.lags[ids] = (
            uniform((len(ids), 3), 0, self.cfg["max_delay_steps"], self.device, self.generator)
            if noisy
            else 0
        )
    ...
    def update(self, state, ids, reset=False, noisy=True):
        ...
        for name, buffer in self.buffers.items():
            ...
            if name.endswith("position"):
                value -= self.position_offset[ids]
            if noisy:
                if name.endswith("quaternion"):
                    ...
                    scale = (
                        self.cfg["joint_noise_rad"] if name == "q" else self.cfg["position_noise_m"]
                    )
                    value += uniform(value.shape, -scale, scale, self.device, self.generator)
            if name == "q":
                value -= self.default_q[ids]
            if reset:
                buffer[ids] = value[:, None]
            else:
            ...
            delayed = (
                slerp(a, b, blend)
                if name.endswith("quaternion")
                else torch.lerp(a, b, blend[:, None])
            )
            if name not in self.delayed:
                self.delayed[name] = torch.zeros_like(state[name])
            self.delayed[name][ids] = delayed
        critic_q = state["q"][ids]
        if self.cfg.get("critic_joint_relative_to_default", False):
            critic_q = critic_q - self.default_q[ids]
        values = dict(
            policy_position=self.delayed["wrist_position"][ids],
            policy_rotation=rotation6d(self.delayed["wrist_quaternion"][ids]),
            policy_q=self.delayed["q"][ids],
            critic_position=state["wrist_position"][ids] - self.position_offset[ids],
            critic_rotation=rotation6d(state["wrist_quaternion"][ids]),
            critic_q=critic_q,
        )
        for name, value in values.items():
            history = self.histories[name]
            if reset:
                history[ids] = value[:, None]
            else:
    ...
    def assemble(self, state, ref, action, phase, ids=None):
        ...
        common = [
            action[ids],
            phase[ids, None],
            ref["wrist_position"][ids] - self.position_offset[ids],
            rotation6d(ref["wrist_quaternion"][ids]),
            ref["q"][ids],
        ]

        def history(prefix):
            return [
                self.histories[prefix + name][ids].flatten(1)
                for name in ("_position", "_rotation", "_q")
            ]

        self.actor[ids] = torch.cat(
            [
                self.delayed["object_position"][ids],
                rotation6d(self.delayed["object_quaternion"][ids]),
                *history("policy"),
                *common,
            ],
            -1,
        ...
        object_velocity = (
            [state["object_velocity"][ids], state["object_angular_velocity"][ids]]
            if self.include_object_velocity
            else []
        )
        self.critic[ids] = torch.cat(
            [
                state["object_position"][ids] - self.position_offset[ids],
                rotation6d(state["object_quaternion"][ids]),
                *history("critic"),
                *common,
                *object_velocity,
                (state["fingertips"][ids] - self.position_offset[ids, None]).flatten(1),
                state["q_velocity"][ids],
            ],
            -1,
        )
        return self.get()
    ...
    def get(self):
        return TensorDict(
            {"policy": self.actor.clone(), "critic": self.critic.clone()}, batch_size=[self.count]
        )
...
class ArmObservations:
    ...
    def __init__(self, count, config, arm, device, generator, max_failures):
        ...
        self.center = tensor((arm.lower + arm.upper) / 2)
        self.half = tensor((arm.upper - arm.lower) / 2)
        self.velocity = tensor(arm.velocity)
        self.error_scale = tensor([0.02] * 3 + [0.2] * 3)
    ...
    def assemble(self, state, ref, action, phase, ids=None):
        ...
        extra = torch.cat(
            (
                (state["q_arm"][ids] - self.center) / self.half,
                ...
                state["arm_ik_failure_streak"][ids, None].float() / self.max_failures,
            ),
            -1,
        )
