...
class ResidualTask:
    ...
    def __init__(self, model, reference, config, device='cpu'):
        ...
        self.critic_observation_size=88+6*int(config['observation'].get('include_object_velocity',False))
    ...
    def score(self,state,ref,action,previous_action,demo_end=None,timed_out=None,*,table_metrics=None):
        ...
        if safety.get('enabled',False):
            if table_metrics is None:
                raise ValueError('Table-safe training requires collision clearance from every physics substep')
            margin=safety['clearance_m']
            actual=((margin-table_metrics['table_clearance_m'])/margin).clamp(0,2)
            command=((margin-table_metrics['table_raw_target_clearance_m'])/margin).clamp(0,3)
            terms.update(table_clearance=-safety['actual_penalty_weight']*actual.square(),
        ...
        metrics=dict(object_keypoint_error_m=eo,wrist_error_m=ep,wrist_rotation_error_rad=er,joint_rmse_rad=eq,coupling_error_rad=coupling,
                     ...
                     object_deviation_failure=object_failure,coupling_failure=coupling_failure,
        ...
        if table_metrics is not None: metrics.update(table_metrics)
