...
class ArmPolicyEnv(PhysxResidualEnv):
    ...
    def __init__(self, root, model, reference, config, arm_config, render=False):
        ...
        if config.get("motion_control", {}).get("enabled", False):
            raise ValueError(
                "Arm policy play requires a checkpoint trained without the optional motion governor"
            )
        root = Path(root)
        self.model, self.reference, self.cfg = model, reference, config
        self.arm_control = dict(
            velocity_feedforward=True,
            restore_hand_materials=True,
            wrist_response="training_pd",
            contact_feedback_gain=0.25,
            contact_filter_alpha=0.5,
        )
        self.arm_control.update(arm_config.get("policy_control", {}))
        if set(self.arm_control) != {
            "velocity_feedforward",
            "restore_hand_materials",
            "wrist_response",
            "contact_feedback_gain",
            "contact_filter_alpha",
        }:
            raise ValueError("Unknown arm policy-control setting")
        if any(
            not isinstance(self.arm_control[name], bool)
            for name in ("velocity_feedforward", "restore_hand_materials")
        ):
        ...
        self.motion_controller = MotionController(
            model, 1, config["physics_dt"], config.get("motion_control"), self.device
        )
        ...
        self.robot = self.world.scene.add(
            Articulation(scene["robot"], name="policy_arm", reset_xform_properties=False)
        )
        self.can = self.world.scene.add(
            RigidPrim(scene["can"], name="policy_can", reset_xform_properties=False)
        )
        self.table = self.world.scene.add(
            RigidPrim(scene["table"], name="policy_table", reset_xform_properties=False)
        )
        self.world.reset()
        if self.robot.count != 1 or not set(self.arm.active_names + model.full_names).issubset(
            self.robot.dof_names
        ):
            raise RuntimeError(
                f"PhysX did not form one combined arm/hand articulation: count={self.robot.count}, joints={self.robot.dof_names}"
            )
        materials = self.robot._physics_view.get_material_properties().detach().cpu().clone()
        materials[..., :2] = config["friction"]
        materials[..., 2] = 0.0
        self.robot._physics_view.set_material_properties(
            materials.contiguous(), torch.zeros(1, dtype=torch.int32)
        )
        ...
        self.arm_ids = torch.tensor(
            [self.robot.dof_names.index(n) for n in self.arm.active_names], device=self.device
        )
        self.body_shape_counts = [
            self.world.physics_sim_view.create_rigid_body_view(path).max_shapes
        ...
        self.robot.set_max_joint_velocities(
            torch.tensor(self.arm.velocity[None], device=self.device, dtype=torch.float32),
            joint_indices=self.arm_ids,
        )
        self.robot.set_max_joint_velocities(
            torch.full(
                (1, len(model.full_names)),
        ...
        if (
            self.arm_control["wrist_response"] == "training_pd"
            and self.arm_control["contact_feedback_gain"] > 0
        ):
            paths = [
                path
                for name, path in zip(self.robot.body_names, self.robot._physics_view.link_paths[0])
                if name in hand_links
            ]
            self.hand_contacts = RigidPrim(
                paths,
                name="policy_hand_contacts",
                reset_xform_properties=False,
                track_contact_forces=True,
                contact_filter_prim_paths_expr=[[] for _ in paths],
                disable_stablization=False,
            )
            self.hand_contacts.initialize(self.world.physics_sim_view)
        self.obs_history = ObservationHistory(
            1, 6, config["observation"], self.device, self.generator
        )
        self.metadata = dict(
            robot="RB3-730 + Revo2",
            num_envs=1,
            ...
            finger_joint_names=model.active_names,
    ...
    def load_training_state_dict(self, state):
        ...
        if self.arm_control["restore_hand_materials"]:
            ...
            source_counts = names.get(
                "body_shape_counts", [counts[name] for name in names["body_names"]]
            )
            materials = named_material_transfer(
                names["body_names"],
                source_counts,
        ...
        methods = dict(
            ...
            masses="set_masses",
            inertias="set_inertias",
            coms="set_coms",
        )
        for key, values in saved["object"].items():
            getattr(self.can._physics_view, methods[key])(
                values[:1].detach().cpu().contiguous(), indices
            )
        self.table._physics_view.set_material_properties(
            saved["table"]["materials"][:1].detach().cpu().contiguous(), indices
        )
        from ..materials import enforce_pad_contact, pad_contact_report
        ...
        self.refresh_physics_properties()
        self.metadata["restored_physics"] = (
            "first supplied checkpoint environment: named hand masses/inertias/drives/default offsets, verified collider materials, can properties and tabletop material"
        )
        if self.arm_control.get("wrist_response", "direct") == "training_pd":
            ...
            relative = (
                ...
                * cache.GetLocalToWorldTransform(root).GetInverse()
            )
            ...
            for name in names["body_names"]:
                source_coms.append(
                    carrier_com
                    if name == "Hand"
                    else actual_coms[self.robot.body_names.index(name)]
                )
            self.wrist_response = WristResponse(
                self.model,
                names["body_names"],
                saved["hand"]["masses"][0].detach().cpu().numpy(),
                saved["hand"]["inertias"][0].detach().cpu().numpy(),
                source_coms,
                self.cfg,
                self.cfg["physics_dt"],
            )
            self.metadata["wrist_response"] = dict(
                mode="training_pd",
                body_names=names["body_names"],
                total_mass_kg=float(self.wrist_response.mass.sum()),
                carrier_com=carrier_com.tolist(),
                physics_dt=self.cfg["physics_dt"],
                hand_asset_sha256=digest(self.hand_asset),
    ...
    def reset(self, indices=None, randomize=False, times=None):
        ...
        if placement_sampler is not None:
            ...
            self.episode_placement = dict(
                grid_index=list(placement.grid_index),
                can_initial_world_xy_m=placement.xy.tolist(),
                world_from_source=self.world_from_source.tolist(),
                base_from_source=placement.base_from_source.tolist(),
                initial_can_pose_world=self.initial_can_pose.tolist(),
                seed=placement_sampler.seed,
            )
        ...
        initial = self.bridge.reset(
            ref["wrist_position"][0].cpu().numpy(), ref["wrist_quaternion"][0].cpu().numpy()
        )
        if placement_sampler is not None:
            self.episode_placement.update(
                initial_q_arm=initial.q.tolist(),
        ...
        p = torch.tensor(
            self.initial_can_pose[None, :3, 3], device=self.device, dtype=torch.float32
        )
        ...
        if self.wrist_response is not None:
            self.wrist_response.reset(
                state["wrist_position"][0].cpu().numpy(), state["wrist_quaternion"][0].cpu().numpy()
            )
    ...
    def step(self, actions, auto_reset=True):
        ...
        position = protected["position"][0].cpu().numpy()
        quaternion = protected["quaternion"][0].cpu().numpy()
        ...
        if response is not None:
            ...
            if self.hand_contacts is not None and float(self.time[0]) > 0:
                forces = (
                    self.frame.rotate(
                        self.hand_contacts.get_net_contact_forces(dt=self.cfg["physics_dt"])
                    )
                    .cpu()
                    .numpy()
                )
                wp, wq = self.hand_contacts.get_world_poses()
                cp, _ = self.hand_contacts.get_coms()
                centers = (
                    self.frame.position(
                        wp + quat_apply(wq[:, [1, 2, 3, 0]], cp.reshape(-1, 3).to(wp.device))
                    )
                    .cpu()
                    .numpy()
                )
                measured = np.r_[
                    forces.sum(0), np.cross(centers - response.position, forces).sum(0)
                ]
            ...
            position, quaternion = response.step(
                position,
                quaternion,
                self.robot.get_joint_positions()[0, self.active_ids].cpu().numpy(),
                self.cfg["control_decimation"],
                contact[:3],
                contact[3:],
            )
        if table is not None:
            response_target = dict(
                target,
                position=torch.as_tensor(position[None], device=self.device, dtype=torch.float32),
                quaternion=torch.as_tensor(
                    quaternion[None], device=self.device, dtype=torch.float32
                ),
            )
            ...
            if response is not None and corrected[2] > position[2] + 1e-7:
                ...
                response.velocity[2] = max(0.0, response.velocity[2])
            position = corrected
        ik = self.bridge.solve(position, quaternion, self.task.dt)
        if response is not None and not ik["success"]:
        ...
        if ik["success"]:
            ...
            self.last_full_target.copy_(target["full_q"])
        ...
        self.robot.set_joint_velocity_targets(
            qva if self.arm_control["velocity_feedforward"] else torch.zeros_like(qva),
            joint_indices=self.arm_ids,
        )
        ...
        measured_pose = self.bridge.target(
            state["wrist_position"][0].cpu().numpy(), state["wrist_quaternion"][0].cpu().numpy()
        )
        ...
        metrics.update(
            gravity_m_s2=scalar(self.gravity.value),
            arm_ik_success=scalar(ik["success"]),
            arm_ik_position_error_m=scalar(result.position_error_m),
            arm_ik_orientation_error_rad=scalar(result.orientation_error_rad),
            arm_sigma_min=scalar(result.sigma_min),
            arm_condition=scalar(result.condition),
            arm_joint_step_rad=scalar(ik["joint_step_rad"].max()),
            arm_joint_limit_distance_rad=scalar(result.joint_limit_distance_rad),
            arm_actual_sigma_min=scalar(actual_sigma),
            arm_actual_condition=scalar(actual_condition),
            arm_joint_limit_violation_rad=scalar(
                max(
                    0.0,
                    np.max(self.arm.lower - measured_arm),
                    np.max(measured_arm - self.arm.upper),
                )
            ),
            arm_joint_tracking_error_rad=scalar(np.max(np.abs(measured_arm - ik["q_arm"]))),
            arm_joint_velocity_violation_rad_s=(
                state["q_arm_velocity"].abs() - torch.tensor(self.arm.velocity, device=self.device)
            )
            .clamp_min(0)
            .amax(-1),
            ...
            policy_wrist_position_error_m=(target["position"] - state["wrist_position"]).norm(
                dim=-1
            ),
            policy_wrist_orientation_error_rad=rotation_error(
                target["quaternion"], state["wrist_quaternion"]
            ).norm(dim=-1),
            ...
            filtered_contact_force_n=scalar(
                np.linalg.norm(getattr(self, "contact_wrench", np.zeros(6))[:3])
            ),
            filtered_contact_moment_nm=scalar(
                np.linalg.norm(getattr(self, "contact_wrench", np.zeros(6))[3:])
            ),
        )
        ...
        applied = dict(
            ...
            full_q=self.last_full_target.clone(),
            q_arm=qa,
            q_arm_velocity=qva
            if self.arm_control["velocity_feedforward"]
            else torch.zeros_like(qva),
        )
        applied_pose = (
            self.frame.source_from_world
            @ self.workcell.world_from_base
            @ self.arm.pose(ik["q_arm"])
        )
        applied["position"] = torch.tensor(
            applied_pose[None, :3, 3], device=self.device, dtype=torch.float32
        )
        applied["quaternion"] = torch.tensor(
            Rotation.from_matrix(applied_pose[:3, :3]).as_quat()[None],
            device=self.device,
            dtype=torch.float32,
        )
        ...
        if self.render:
            self.world.render()
            time.sleep(max(0.0, self.task.dt - (time.monotonic() - started)))
