...
class PPO:
    def __init__(self,task,config,device='cpu',num_envs=1):
        ...
        dummy=TensorDict({'policy':torch.zeros(num_envs,task.observation_size,device=device),
                          'critic':torch.zeros(num_envs,task.critic_observation_size,device=device)},batch_size=[num_envs])
        groups={'actor':['policy'],'critic':['critic']}
        common=dict(obs=dummy,obs_groups=groups,hidden_dims=config['hidden_sizes'],activation=config['activation'],obs_normalization=config['observation_normalization'])
        self.actor=MLPModel(**common,obs_set='actor',output_dim=task.action_size,
                            distribution_cfg={'class_name':'GaussianDistribution','init_std':config['initial_std'],'std_type':'scalar'}).to(device)
        ...
        storage=RolloutStorage('rl',num_envs,config['rollout_steps'],dummy,[task.action_size],device=str(device))
        self.algorithm=LibraryPPO(self.actor,self.critic,storage,
            num_learning_epochs=config['epochs'],num_mini_batches=config['num_minibatches'],clip_param=config['clip_ratio'],gamma=config['gamma'],lam=config['gae_lambda'],
            value_loss_coef=config['value_coefficient'],entropy_coef=config['entropy_coefficient'],learning_rate=config['learning_rate'],max_grad_norm=config['max_grad_norm'],
            schedule=config['schedule'],desired_kl=config['target_kl'],use_clipped_value_loss=True,normalize_advantage_per_mini_batch=False,device=str(device))
    ...
    def collect(self,env,observation):
        ...
        with torch.no_grad():
            for _ in range(self.cfg['rollout_steps']):
                ...
                if timeout.any():
                    adjusted += self.cfg['gamma']*self.critic(info['final_observation'].to(self.device)).squeeze(-1)*timeout
                # Already corrected with V(final state), so do not pass RSL's V(s_t) timeout branch.
                self.algorithm.process_env_step(observation,adjusted,(term|trunc).to(self.device),{})
    ...
    def save(self,path,metadata,training_state=None):
        ...
        torch.save(dict(schema=3,learner=self.algorithm.save(),learning_rate=self.algorithm.learning_rate,
                        config=self.cfg,iteration=self.iteration,steps=self.steps,torch_rng_state=torch.get_rng_state(),
                        cuda_rng_states=torch.cuda.get_rng_state_all() if torch.cuda.is_available() else [],
                        metadata=metadata,training_state=training_state),temporary)
        temporary.replace(path)

    def load(self,path,expected_metadata,resume=False):
        data=torch.load(path,map_location=self.device,weights_only=True)
        if data.get('schema')!=3 or data['metadata']['contract_hash']!=expected_metadata['contract_hash']:
            raise ValueError('Checkpoint reference/model/control/observation contract mismatch; v2 policies are incompatible')
        if data['config']!=self.cfg: raise ValueError('PPO config changed')
        self.algorithm.load(data['learner'],{'actor':True,'critic':True,'optimizer':resume},strict=True)
        self.iteration,self.steps=data['iteration'],data['steps']
        if resume:
            self.algorithm.learning_rate=data['learning_rate']
            for group in self.optimizer.param_groups: group['lr']=data['learning_rate']
    ...
    def export(self,directory):
        ...
        torch.onnx.export(onnx,onnx.get_dummy_inputs(),str(directory/'policy.onnx'),opset_version=18,
                          input_names=onnx.input_names,output_names=onnx.output_names,dynamic_axes={'obs':{0:'batch'},'actions':{0:'batch'}},dynamo=False)
