...
class ReferenceStateSampler:
    ...
    def probabilities(self):
        ...
        if self.cfg['kernel_size']>1:
            kernel=self.cfg['kernel_decay']**torch.arange(self.cfg['kernel_size'],device=self.device)
    ...
    def record(self,times,terminated,demo_end,num_envs=None,max_episode_steps=None):
        bins=((times/self.dt).round().long()*self.bin_count//self.frame_count).clamp(0,self.bin_count-1)
        self.episodes+=int((terminated|demo_end).sum())
        if self.cfg['sampling']!='adaptive': return
        counts=torch.bincount(bins[terminated&~demo_end],minlength=self.bin_count).float()
        effective_alpha=min(self.cfg['ema_alpha']*(num_envs or len(times))/(max_episode_steps or (self.frame_count-1)),1.)
    ...
    def load_state_dict(self,state):
        ...
        if values.shape!=self.failures.shape or not torch.isfinite(values).all() or (values<0).any(): raise ValueError('Invalid RSI histogram')
        self.failures.copy_(values);self.episodes=int(state['episodes'])
...
class GravityCurriculum:
    def __init__(self, config, evaluation_gravity):
        ...
        if (self.stages.ndim != 2 or self.stages.shape[1] != 3 or self.stages[0, 0] != 0
                or not np.isfinite(self.stages).all() or np.any(np.diff(self.stages[:, 0]) <= 0)
                or np.any(self.stages[:, 1] < 0) or np.any(self.stages[:, 1] > self.stages[:, 2])
                or np.any(np.diff(self.stages[:, 1:], axis=0) < 0)):
            raise ValueError("Gravity stages must be increasing [control_step, min, max] starting at zero")
    ...
    def sample(self, control_steps, rng, training):
        ...
        self.value = float(rng.uniform(low, high)) if training and self.cfg["enabled"] else float(self.evaluation_gravity)
        return self.value
