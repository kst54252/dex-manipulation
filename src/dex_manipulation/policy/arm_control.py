...
class WristResponse:
    def __init__(self, hand_model, body_names, masses, inertias, coms, gains, physics_dt):
        ...
        if (len(set(self.names))!=count or self.mass.shape!=(count,) or self.inertia.shape!=(count,3,3)
                or self.coms.shape!=(count,7) or not np.isfinite(self.mass).all()
                or not np.isfinite(self.inertia).all() or not np.isfinite(self.coms).all()
                or np.any(self.mass<=0) or np.any(np.linalg.eigvalsh(self.inertia)<=0)
        ...
        self.kp=float(gains['root_kp_position']);self.kd=float(gains['root_kd_position'])
        self.kr=float(gains['root_kp_rotation']);self.dr=float(gains['root_kd_rotation'])
    ...
    def reset(self, position, quaternion):
        ...
        self.velocity=np.zeros(3);self.omega=np.zeros(3)
    ...
    def composite(self, q):
        ...
        total=self.mass.sum();center=np.sum(self.mass[:,None]*centers,axis=0)/total
        offsets=centers-center
        inertia=sum(I+m*(np.eye(3)*np.dot(r,r)-np.outer(r,r))
                    for m,r,I in zip(self.mass,offsets,inertias))
        return total,center,inertia

    def step(self, position, quaternion, measured_fingers, substeps, contact_force=None, contact_torque=None):
        ...
        """
        if self.position is None:raise RuntimeError('Reset the wrist response first')
        ...
        if target.shape!=(3,) or not np.isfinite(target).all():raise ValueError('Invalid wrist target')
        desired=Rotation.from_quat(quaternion)
        external_force=np.zeros(3) if contact_force is None else np.asarray(contact_force,dtype=float)
        external_torque=np.zeros(3) if contact_torque is None else np.asarray(contact_torque,dtype=float)
        if (external_force.shape!=(3,) or external_torque.shape!=(3,)
                or not np.isfinite(external_force).all() or not np.isfinite(external_torque).all()):
        ...
        for _ in range(int(substeps)):
            ...
            acceleration=(force+external_force)/mass-np.cross(alpha,radius)-np.cross(self.omega,np.cross(self.omega,radius))
            self.velocity+=acceleration*self.dt
            self.position+=self.velocity*self.dt
            self.omega+=alpha*self.dt
            self.rotation=(Rotation.from_rotvec(self.omega*self.dt)*Rotation.from_matrix(rotation)).as_matrix()
