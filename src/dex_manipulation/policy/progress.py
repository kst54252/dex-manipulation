...
class RolloutMetrics:
    ...
    def add(self, reward, terminated, truncated, metrics):
        ...
        values = torch.stack(
            [metrics[k].detach().to(self.returns.device, torch.float32) for k in names]
        )
        ...
        self.counts += torch.stack(
            [failure.sum(), complete.sum(), (truncated & ~terminated).sum(), done.sum()]
        )
    ...
    def finish(self):
        ...
        failures, completed, timeouts, episodes, reward = (
            torch.cat((self.counts, self.reward_sum[None] / self.samples)).cpu().tolist()
        )
        ...
        ended = (
            torch.stack(self.finished)[torch.stack(self.done_masks)][-self.episodes.maxlen :]
            .cpu()
            .tolist()
        )
        ...
        return dict(
            mean_object_error_m=metrics["object_keypoint_error_m"]["mean"],
...
def timing_stats(
    *, elapsed, completed, iterations, start_iteration, collection, learning, iteration, transitions
):
    ...
    return dict(
        iteration_in_run=completed,
        iterations_in_run=iterations,
        target_iteration=start_iteration + iterations,
        elapsed_s=elapsed,
        eta_s=remaining,
        estimated_total_s=elapsed + remaining,
...
def format_progress(stats, *, num_envs, control_dt, rollout_steps, decimation, rsi, augmentation):
    ...
    lines = [
        line,
        f"Learning iteration {stats['iteration']}/{stats['target_iteration']}"
        ...
        f"  |  Estimated total {_duration(stats['estimated_total_s'])}",
        "-" * width,
    ]
    lines.append(
        f"Step reward {_number(stats['mean_step_reward'])}  |  Failures {stats['failures']}"
        f"  |  Demo ends {stats['completed']}  |  Timeouts {stats['timeouts']}"
    )
    episode = stats["episode"]
    lines.append(
        f"Episode return {_number(episode['mean_return'])}  |  Length {_number(episode['mean_length_steps'])} steps"
    ...
    lines.append("  |  ".join(f"{k} {_number(stats[k])}" for k in loss_keys[3:] if k in stats))
    lines.append(
        f"Gravity {stats['gravity_m_s2']:.3f} m/s^2 (stage {stats['gravity_stage']})"
    ...
    lines.extend(
        [
            "-" * width,
            f"{'Rollout metrics (all environments and steps)':<55} {'mean':>16} {'max':>16}",
        ]
    )
    ...
    lines.extend(
        [
            "-" * width,
            f"{'Weighted reward contributions (including control dt)':<55} {'mean/step':>16} {'max/step':>16}",
        ]
    )
    for key, values in stats["metrics"].items():
        if key.startswith("reward_"):
            lines.append(
                f"{key:<55} {_number(values['mean'] * control_dt):>16} {_number(values['max'] * control_dt):>16}"
            )
