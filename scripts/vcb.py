...
def main(argv=None):
    ...
    parser.add_argument('mode',choices=('init','probe','plan','prepare','bridge','send','mirror','status'))
    ...
    parser.add_argument('--arm-sign',type=float,nargs=6,help='Verified USD-to-VCB axis signs, for init')
    parser.add_argument('--arm-offset-deg',type=float,nargs=6,help='Verified USD-to-VCB zero offsets, for init')
    parser.add_argument('--recording',type=Path)
    parser.add_argument('--ros-config',type=Path,default=ROOT/'config/ros_vcb.json')
    parser.add_argument('--read-only',action='store_true',help='Bridge without accepting motion goals')
    ...
    try:
        ...
        if args.mode=='init':
            if not path.is_relative_to(ROOT/'local'): raise ValueError('Keep VM settings under local/')
            config = json.loads((ROOT/'config/vcb.example.json').read_text())
            config['rb3']['address'] = args.address
            connection_plan(config)
            if (args.arm_sign is None)!=(args.arm_offset_deg is None):
                raise ValueError('Provide both axis signs and zero offsets, or leave both for later')
            ...
            with path.open('x') as file: file.write(json.dumps(config,indent=2)+'\n')
        ...
        if args.address or args.arm_sign is not None or args.arm_offset_deg is not None:
            raise ValueError('Address/mapping options belong to init; otherwise edit the selected config')
        if args.read_only and args.mode!='bridge': raise ValueError('--read-only belongs to bridge')
        if args.mode in ('bridge','send','mirror','status'):
            from scripts.ros import main as ros_main
            forwarded = [args.mode,'--config',str(args.ros_config),'--seconds',str(args.seconds)]
            if args.mode=='bridge':
                forwarded += ['--backend','vcb','--vcb-config',str(path)]
                if not args.read_only: forwarded += ['--enable-motion']
            if args.recording: forwarded += ['--recording',str(args.recording)]
            if args.headless: forwarded += ['--headless']
            return ros_main(forwarded)
        config = json.loads(path.read_text())
        if args.mode=='probe': report = asyncio.run(probe(config))
        else:
    ...
    except (OSError,ValueError,KeyError,TypeError,RuntimeError,ImportError,TimeoutError) as error:
