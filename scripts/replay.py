...
def main():
    ...
    try:
        ...
        if reference.metadata.get('object_geometry_fingerprint') != can_geometry.get('fingerprint'):
            raise ValueError('Arm reference uses stale object geometry. Regenerate grounding and IK.')
        import hashlib
        if can_geometry.get('collision_source_sha256') != hashlib.sha256((ROOT/can_geometry['collision_source']).read_bytes()).hexdigest():
            raise ValueError('Object USD changed since model extraction. Rebuild object and reference.')
        arm = ArmModel.load(ROOT/config['arm_model']); hand = HandModel.load(ROOT/config['hand_model'])
        if (model_fingerprint(arm) != reference.metadata['arm_fingerprint'] or
                model_fingerprint(hand) != reference.metadata['hand_fingerprint']):
            raise ValueError('Reference arm/mount/hand model differs from replay models')
        import hashlib
        if hashlib.sha256((ROOT/config['usd']).read_bytes()).hexdigest() != arm.description['source_sha256']:
        ...
        world = World(stage_units_in_meters=1.,physics_dt=1/args.rate,rendering_dt=1/args.rate)
        ...
        for prim in stage.Traverse():
            ...
            if prim.IsA(UsdPhysics.RevoluteJoint) and args.mode=='kinematic':
                ...
                if drive:drive.CreateStiffnessAttr(0.);drive.CreateDampingAttr(0.)
        ...
        if args.mode=='targets':
            ...
            for joint in hand.moving:
                ...
                api.CreateReferenceJointAxisAttr('rot'+str(UsdPhysics.RevoluteJoint(leader).GetAxisAttr().Get()))
                api.CreateGearingAttr(-spec['multiplier']);api.CreateOffsetAttr(-np.rad2deg(spec['offset']))
        ...
        if object_data is not None:
            if args.mode=='targets':
                ...
                dynamic_can=world.scene.add(RigidPrim('/DynamicCan',name='free_can',
                    positions=initial_can_pose[None,:3,3],orientations=quat[None]))
            else:
        ...
        if arm.wrist not in bodies:raise ValueError(f'Mounted hand is not in RB3 articulation: {bodies}')
        ...
        for loop in range(args.loops):
            if args.mode=='targets':
                ...
                if dynamic_can is not None:
                    ...
                    initial=transform(Rotation.from_quat(orientation[0][[1,2,3,0]]).as_matrix(),pos[0])
                    initial_bottoms.append(collision_bottom(initial,can_shapes))
                    if (reference.metadata.get('alignment_status')=='simulation_ground_placement_not_calibration'
            ...
            for index,target in enumerate(targets):
                ...
                if args.mode=='kinematic':adapter.set_state(target)
                else:adapter.set_target(target)
                if can_ops is not None:
                    pos=np.array([np.interp(target.timestamp_s,reference.times,object_data[:,i,3]) for i in range(3)])
                ...
                row=dict(time_s=target.timestamp_s,position_error_m=float(np.linalg.norm(error[:3])),
                ...
                if dynamic_can is not None:
                    ...
                    if not np.isfinite(position).all():raise RuntimeError('Non-finite can state')
                    row.update(can_x_m=float(position[0]),can_y_m=float(position[1]),can_z_m=float(position[2]))
                if loop==0:
                    records.append(row)
                    matches=np.flatnonzero(np.isclose(reference.times,target.timestamp_s,atol=1e-8))
                    matches=matches[source_mask[matches]]
                    if len(matches):
                        i=int(matches[0]);e=pose_error(reference.data['wrist_target_pose'][i],measured)
                        frame_records.append(dict(frame_id=int(reference.data['frame_ids'][i]),position_error_m=float(np.linalg.norm(e[:3])),orientation_error_rad=float(np.linalg.norm(e[3:]))))
                        if args.capture:
                            for _ in range(3):world.render()
                            file=args.output/'frames'/f"frame_{int(reference.data['frame_ids'][i]):06d}.png"
                            ...
                            while time.monotonic()<deadline:
                                ...
                                if future.done():
                                    future.result()
                                    if file.exists() and file.stat().st_size>8:break
                                time.sleep(.01)
                            if not future.done() or not file.exists():raise RuntimeError('Viewport capture did not complete')
                if args.realtime:
                    deadline=started+loop*(reference.times[-1]-reference.times[0])+target.timestamp_s-reference.times[0]
                    delay=deadline-time.monotonic()
                    if delay>0:time.sleep(min(delay,1/args.rate))
            if not app.is_running():break
        if not records:raise RuntimeError('No frames replayed')
        success=(len(records)==len(targets) and len(frame_records)==int(source_mask.sum()) and
        ...
        report=dict(mode=args.mode,samples=len(records),source_frames=len(frame_records),ik_samples=len(reference.times),loops_requested=args.loops,
                    ...
                    frame_measurements=frame_records,alignment_status=reference.metadata['alignment_status'],
        ...
        np.savez_compressed(args.output/'measurements.npz',**{k:np.array([r[k] for r in records]) for k in records[0]})
        print('[replay]',json.dumps(report),flush=True)
        if args.mode=='kinematic' and not success:code=2
        if args.mode=='targets' and len(records)!=len(targets):code=2
    except Exception:
        import traceback
