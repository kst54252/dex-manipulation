...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Repeat complete demonstrations; 0 repeats until stopped",
    )
    parser.add_argument(
        "--speed",
        type=float,
        default=1.0,
        help="Floating reference speed multiplier; physics dt stays fixed",
    )
    ...
    parser.add_argument(
        "--arm-reference", type=Path, default=ROOT / "local/results/ik/full/trajectory.npz"
    )
    args = parser.parse_args()
    if not math.isfinite(args.speed) or args.speed <= 0:
        parser.error("--speed must be positive and finite")
    if args.robot == "arm" and args.speed != 1.0:
        parser.error("--speed currently applies only to floating playback")
    if args.loops < 0:
        parser.error("--loops must be nonnegative (0 repeats until stopped)")
    ...
    if args.robot == "arm":
        ...
        if reason:
            parser.error(
                f"Arm reference unavailable/stale: {reason}. Use ./run.sh arm retarget for automatic IK preparation."
            )
