...
def main():
    ...
    parser.add_argument('--alignment', type=Path, help='JSON with world_from_source or measured base_from_source and status')
    parser.add_argument('--max-frames', type=int)
    parser.add_argument('--trajectory-substeps', type=int, help='IK samples per original interval; preserve original times/poses')
    parser.add_argument('--seed', nargs=6, type=float, metavar='RAD', help='Explicit initial q in USD joint order')
    args = parser.parse_args()
    config = json.loads(args.config.read_text())
    workcell = Workcell.load(ROOT/config['workcell'])
    if args.command == 'extract':
        from dex_manipulation.usd import extract_arm
        extract_arm(ROOT/config['usd'], config['base_path'], config['flange_path'], config['wrist_path'], ROOT/config['arm_model'])
    ...
    with np.load(source, allow_pickle=False) as data:
        grounded = ('frame_metadata_json' in data and
                    json.loads(str(data['frame_metadata_json']))['coordinate_frame'] == 'ground')
    if grounded and alignment_path is not None:
        specified = workcell.resolve_alignment(json.loads(alignment_path.read_text()))
        matrix = checked_pose(specified['world_from_source'])
        if (specified['status'].startswith('simulation') and
                not np.allclose(matrix[2], [0,0,1,0], atol=1e-9, rtol=0)):
            parser.error('Grounded simulation placement must preserve tabletop world Z=0.')
    if args.command == 'place':
        if grounded:
            if alignment_path is None:
                parser.error('Grounded input requires an explicit planar alignment; wrist-based placement would lift the can.')
        ...
        args.output.mkdir(parents=True, exist_ok=True)
        path = args.output/'alignment.json'
        path.write_text(json.dumps(dict(base_from_source=alignment.tolist(), status='simulation_placement_not_calibration',
            method='single rigid transform mapping source first wrist to FK(seed); all relative poses preserved',
            initial_seed_q_rad=seed.tolist(), hardware_calibrated=False), indent=2)+'\n')
        print(path); return
    if alignment_path is None:
        parser.error('Specify --alignment. Camera coordinates cannot silently be treated as RB3 base coordinates. Use place for an explicit simulation scenario.')
    alignment = workcell.resolve_alignment(json.loads(alignment_path.read_text()))
    report = solve_trajectory(model,hand,source,args.output,alignment['base_from_source'],seed,
                              IKOptions(**config['solver']),alignment['status'],args.max_frames,
                              args.trajectory_substeps if args.trajectory_substeps is not None else config.get('trajectory_substeps',1),
    ...
    print(json.dumps({k:report[k] for k in ('success_count','frame_count','continuous_reference_valid','failed_frame_ids')}))
