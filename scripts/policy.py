...
def parse_args(argv=None, modes=("train", "evaluate", "play"), default_mode="train"):
    ...
    parser.add_argument(
        "--robot",
        choices=("floating", "arm"),
        default="floating",
        help="Floating hand or physical RB3+Revo2; arm training needs an arm_training config",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Play mode: repeat until stopped (0), or stop after this many episodes",
    )
    parser.add_argument(
        "--speed",
        type=float,
        default=1.0,
        help="Play reference speed multiplier for floating/arm; physics and control dt stay fixed",
    )
    parser.add_argument(
        "--random-can",
        action="store_true",
        help="Demo 2 arm play: resample a verified tabletop IK point on every episode reset",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        choices=("cpu", "cuda:0"),
        default=None,
        help="Physics and default policy device",
    )
    parser.add_argument(
        "--export",
        action="store_true",
        help="Export the checked checkpoint to JIT and ONNX after evaluation",
    )
    ...
    parser.add_argument(
        ...
        choices=("pretty", "json"),
        default="pretty",
        help="Per-iteration terminal format; training.jsonl always contains full statistics",
    )
    parser.add_argument(
        "--source-profile",
        choices=("revo2", "leap", "wuji"),
        default=None,
        help="Recipe label/profile; default uses the config file's source_profile; hardware remains Revo2",
    )
    parser.add_argument(
        "--rsi-sampling", choices=("uniform_frames", "uniform", "adaptive"), default=None
    )
    rsi_flags = parser.add_mutually_exclusive_group()
    rsi_flags.add_argument(
        "--enable-rsi",
        action="store_true",
        help="Enable random reference-state starts (on in the source-aligned default config)",
    )
    rsi_flags.add_argument(
        "--disable-rsi", action="store_true", help="Always start from the first reference frame"
    )
    ...
    parser.add_argument(
        "--skip-evaluation",
        action="store_true",
        help="Train only; omit full-gravity baseline and final evaluation",
    )
    parser.add_argument(
        "--evaluation-protocol",
        choices=("source", "strict", "both"),
        default="both",
        help="Source PLAY noise/augmentation and 1 m deviation threshold; strict adds a fixed-reference diagnostic",
    )
    parser.add_argument(
        "--motion-control",
    ...
    if args.mode == "play":
        ...
        if args.output == ROOT / "local/results/policy":
            args.output = ROOT / (
                "local/results/policy/play_arm"
                if args.robot == "arm"
                else "local/results/policy/play"
            )
