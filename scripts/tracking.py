...
def main():
    ...
    plan = sub.add_parser(
        "plan", help="Convert a strict floating capture into saved arm+finger targets"
    )
    ...
    plan.add_argument("--time-scale", type=float, default=1.0)
    plan.add_argument("--output", type=Path, required=True)
    play = sub.add_parser(
        "replay", help="Drive the physical arm using the saved joint trajectory only"
    )
    ...
    play.add_argument(
        ...
        type=Path,
        required=True,
        help="Restore saved physical properties only; no actor inference",
    )
    play.add_argument("--output", type=Path, required=True)
    play.add_argument("--repeats", type=int, default=3)
    play.add_argument("--hold", type=float, default=1.0)
    ...
    if args.mode == "plan":
        ...
        report = make_plan(
            ROOT, args.capture, args.arm_config, args.output, args.kind, args.time_scale
        )
        print(
            json.dumps(
                {
    ...
    manifest = read_config(args.reference.parent / "frozen.json")
    ...
    def ready(env, metadata):
        ...
        return replay(
            ...
            args.repeats,
            args.hold,
            not args.no_velocity_feedforward,
        )
