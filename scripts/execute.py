...
def _main(argv=None):
    ...
    parser.add_argument('--checkpoint', type=Path, help='record: latest completed demo policy by default; replay: recorded checkpoint')
    ...
    parser.add_argument('--gui', action='store_true', help='Show the requested simulation (default headless)')
    parser.add_argument('--hardware-config', type=Path, default=ROOT/'config/hardware.example.json')
    parser.add_argument('--send', action='store_true', help='Actually connect and stream to commissioned hardware')
    args = parser.parse_args(argv)
    for name in ('recording', 'checkpoint', 'arm_config', 'hardware_config', 'output'):
        value = getattr(args, name)
        if value is not None:
            setattr(args, name, (ROOT/value).resolve())
    if args.send and args.mode != 'hardware': parser.error('--send requires hardware mode')
    if args.hold <= 0: parser.error('--hold must be positive')
    ...
    if args.mode == 'inspect':
        print(json.dumps(frozen.summary(), indent=2)); return 0
    output = (args.output or ROOT/('local/results/execution/' + args.mode+'_'+datetime.now().strftime('%Y%m%d_%H%M%S_%f'))).resolve()
    if not output.is_relative_to(ROOT/'local') or output.exists():
        parser.error('Use a new output directory under local/')
    if args.mode == 'dry-run':
        result = asyncio.run(stream(frozen, MockBackend(frozen.initial), output,
            start_tolerance_rad=[.01]*12, tracking_tolerance_rad=[.01]*12,
            maximum_feedback_age_s=.1, maximum_lateness_s=.02, hold_s=args.hold))
    ...
    if args.mode == 'hardware':
        ...
        print(json.dumps(asyncio.run(execute(frozen, config, output, args.hold)), indent=2)); return 0
    output.mkdir(parents=True)
    config = (frozen.metadata['config'] if frozen else json.loads((args.checkpoint.parent/'config.resolved.json').read_text()))
    ...
    sim_args = parse_args(['--mode','play','--robot','arm','--episodes','1','--speed','1',
        '--evaluation-protocol','strict','--contact-materials','checkpoint','--table-safety','checkpoint',
        '--checkpoint',str(args.checkpoint),'--config',str(config_path),
        '--arm-config',str(args.arm_config),'--output',str(output)] + ([] if args.gui else ['--headless']))
    def ready(env, metadata):
        from dex_manipulation.execution_sim import run
        return run(env, metadata, args.checkpoint, output,
                   replay_path=args.recording if args.mode == 'replay' else None, hold_s=args.hold)
