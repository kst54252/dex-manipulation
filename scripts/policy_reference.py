...
from dex_manipulation.policy.prepare import prepare_contact_reference
...
def main():
    ...
    a=p.parse_args()
    report=prepare_contact_reference(a.reference,a.rollout,a.output,HandModel.load(a.model),a.object_geometry,environment=a.environment)
    print(json.dumps(dict(output=str(a.output),frames=len(report['frames']),
        object_targets_changed=False,solver_status_failures=[r['frame_id'] for r in report['frames'] if not r['solver_success']]),indent=2))
